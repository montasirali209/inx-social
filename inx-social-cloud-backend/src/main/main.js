...
function registerIpc() {
  ...
  ipcMain.handle('account:register', async (_, payload) => {
    ...
    try {
      const result = await client.register({ name: payload.name, email: payload.email, password: payload.password, businessName: payload.businessName || '', marketingOptIn: Boolean(payload.marketingOptIn), acceptedTerms: true });
      appStore.log('account', `Created unverified cloud account for ${result.email}.`);
      return { requiresVerification: true, email: result.email, message: result.message, devVerificationToken: result.devVerificationToken || null, state: appStore.getState() };
