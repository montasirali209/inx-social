...
function dosTimestamp(date = new Date()) {
  ...
  const stamp = (((year - 1980) & 127) << 9) | (((date.getMonth() + 1) & 15) << 5) | (Math.max(1, date.getDate()) & 31);
...
async function downloadUgcAvatars(req, res, next) {
  try {
    ...
    const result = await ugcAdminAvatars.systemAvatarFiles(ids);
...
async function deleteUgcAvatars(req, res, next) {
  try {
    const { ids } = ugcAvatarSelectionSchema.parse(req.body || {});
    const avatars = await ugcAdminAvatars.disableSystemAvatars(ids);
    await prisma.auditLog.create({
      data: {
        userId: req.user.id,
...
async function ugcOperationsSummary(req, res, next) {
  try {
    const limit = z.coerce.number().int().min(1).max(40).default(20).parse(req.query.limit);
    res.json({ operations: await ugcProductionAudit.operationsSummary({ limit }) });
  } catch (err) { next(err); }
}
