...
function completionPage(res, payload) {
  ...
  res.type('html').send(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>INXSocial connection</title></head><body style="margin:0;background:#06131f;color:#e2e8f0;font:16px system-ui;display:grid;min-height:100vh;place-items:center"><main style="max-width:520px;padding:32px;text-align:center"><h1>${payload.ok ? 'Account connected' : 'Connection failed'}</h1><p>${payload.ok ? 'Your account is connected. You can return to INXSocial.' : 'Return to INXSocial and try the connection again.'}</p></main><script>const payload=${safePayload};try{window.opener?.postMessage(payload,'*');localStorage.setItem('inx-social-oauth-result',JSON.stringify(payload));}catch(_){}setTimeout(()=>window.close(),900);</script></body></html>`);
}
...
async function list(req, res, next) {
  try {
    ...
    res.json({
      connections: await postForMe.listConnections(req.user.id),
      providers: postForMe.providerState(),
...
async function disconnect(req, res, next) {
  try {
    res.json(await postForMe.disconnect(req.user.id, req.params.id));
  } catch (error) { next(error); }
}
...
async function webhook(req, res) {
  try {
    ...
    // Acknowledge immediately; processing is idempotent and provider retries are
    // reserved for actual transport failures, not our database work.
    res.status(202).json({ ok: true });
    setImmediate(async () => {
      try {
        const accountHandled = await postForMe.handleAccountWebhook(payload);
        if (!accountHandled) await publishing.handleWebhook(payload);
      } catch (error) {
        console.error('[post-for-me] webhook processing failed:', error?.message || error);
      }
    });
  } catch (error) {
    return res.status(Number(error.status || 500)).json({ error: error.publicMessage || error.message || 'Webhook verification failed.' });
  }
}

module.exports = { list, start, sync, disconnect, callback, webhook };
