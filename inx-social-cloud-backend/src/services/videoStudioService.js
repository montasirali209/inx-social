...
function catalog() {
  return [
    { id: 'fast', name: 'P-Video', badge: 'Fast', description: 'Fast social clips and rapid iteration. Best for testing motion ideas before a premium render.', resolutions: ['720p', '1080p'], durations: [5, 10], aspects: ['9:16', '16:9', '1:1'], draftSupported: true, audioSupported: true, imageReferenceSupported: true },
    { id: 'quality', name: 'Wan 3.0', badge: 'Quality', description: 'Higher-fidelity generation for hero reels, product storytelling and reference-led motion.', resolutions: ['480p', '720p', '1080p'], durations: [5, 10, 15], aspects: ['9:16', '16:9', '1:1'], draftSupported: false, audioSupported: true, imageReferenceSupported: true }
  ];
}
function modelConfig(route) {
  return String(route || 'fast') === 'quality' ? { route: 'quality', model: env.runware.videoLongModel || 'alibaba:wan@3.0' } : { route: 'fast', model: env.runware.videoModel || 'prunaai:p-video@0' };
}
function estimateCredits(input = {}) {
  ...
  if (route === 'fast') creditsPerSecond = draft ? (resolution === '1080p' ? 1.5 : 1) : (resolution === '1080p' ? 4 : 2);
  else creditsPerSecond = resolution === '1080p' ? 20 : resolution === '720p' ? 10 : 5;
...
function dimensions(route, resolution, aspect) {
  ...
  const quality = {
    '480p:16:9': [832, 480], '480p:9:16': [480, 832], '480p:1:1': [624, 624],
...
async function providerGenerate(input, reference, onProgress) {
  ...
  const draft = config.route === 'fast' && Boolean(input.draft);
  const taskUUID = crypto.randomUUID();
  const task = {
    taskType: 'videoInference', taskUUID, model: config.model, deliveryMethod: 'async', positivePrompt: clean(input.prompt, config.route === 'fast' ? 2000 : 10000), duration,
    includeCost: true, outputType: 'URL', settings: config.route === 'fast' ? { audio: input.audio !== false, draft, promptUpsampling: true } : { audio: input.audio !== false, promptExtend: true }
  };
  if (reference) {
    task.inputs = config.route === 'quality' ? { referenceImages: [reference.dataUri] } : { frameImages: [reference.dataUri] };
    task.resolution = resolution;
  } else {
    const [width, height] = dimensions(config.route, resolution, aspect); task.width = width; task.height = height;
  }
  if (config.route === 'fast') task.fps = 24;
  onProgress(5);
  const initial = await runware.request([task], 60000);
  const first = initial.find(entry => entry.taskUUID === taskUUID) || initial[0];
  if (first?.videoURL) return { item: first, taskUUID, route: config.route, model: config.model, duration, resolution, aspect };
  const final = await poll(taskUUID, Number(env.runware.videoTimeoutMs || 600000), onProgress); onProgress(100);
  return { item: final, taskUUID, route: config.route, model: config.model, duration, resolution, aspect };
}
...
async function generateVideo(userId, input = {}) {
  ...
  const config = modelConfig(input.modelRoute); const available = catalog().find(item => item.id === config.route);
  const duration = Math.floor(Number(input.duration || 5)); const resolution = String(input.resolution || '720p'); const aspect = String(input.aspectRatio || '9:16');
  ...
  const amount = estimateCredits({ ...input, duration, resolution, modelRoute: config.route }); await credits.getBalance(userId);
  ...
  try {
    ...
    const output = await providerGenerate({ ...input, duration, resolution, aspectRatio: aspect, modelRoute: config.route }, reference, progress => { void prisma.$executeRawUnsafe('UPDATE "AiGeneration" SET "status"=$2,"progress"=$3,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$1', generationId, 'PROCESSING', Math.max(5, Math.min(95, progress))).catch(() => {}); });
