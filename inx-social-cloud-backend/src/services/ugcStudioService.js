...
async function systemAvatarFiles(avatarIds = []) {
  const ids = [...new Set((Array.isArray(avatarIds) ? avatarIds : []).map(value => clean(value, 120)).filter(Boolean))].slice(0, 200);
  if (!ids.length) throw publicError('Select at least one creator.', 'UGC_ADMIN_AVATAR_SELECTION_REQUIRED', 400);
...
async function warmSystemAvatarReferences() {
  ...
  try {
    const pending = await prisma.$queryRawUnsafe(
      'SELECT * FROM "UGCAvatar" WHERE "scope"=\'SYSTEM\' AND "status"=\'READY\' AND "featured"=true AND ("referenceStorageKey" IS NULL OR "referenceVersion" < $1) ORDER BY "name"', FEATURED_REFERENCE_VERSION
    );
...
module.exports = {
  ...
  listSystemAvatars, uploadSystemAvatar, getSystemAvatarContent, systemAvatarFiles, disableSystemAvatars,
 ...
 + (index + 1)).join(',');
  ...
  for (const row of rows) {
    ...
    try {
      ...
      files.push({ id: row.id, name: safeName + '-' + String(row.id).slice(0, 8) + extension, data, mimeType });
    } catch (_) {
      missing.push({ id: row.id, name: row.name });
    }
  }
...
module.exports = {
 ...
 + (index + 1)).join(',');
  ...
  return rows.map(row => ({ id: row.id, name: row.name, managedByAdmin: String(row.slug || '').startsWith('admin-ugc-') }));
}
