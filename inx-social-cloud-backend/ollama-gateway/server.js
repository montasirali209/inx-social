...
const config = {
  ...
  timeoutMs: Number(process.env.GATEWAY_TIMEOUT_MS || 180000)
};
let active = 0;
const queue = [];
...
async function proxy(req, res, ollamaPath) {
  const body = await readJson(req);
  if (!config.models.has(String(body.model || ''))) return json(res, 400, { error: 'Model is not allowed by this gateway.' });
  body.stream = false;
  await acquire();
  try {
...
function createServer() {
  ...
  return http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && req.url === '/health') return json(res, 200, { ok: true, service: 'inx-ollama-gateway', active, queued: queue.length, models: [...config.models] });
