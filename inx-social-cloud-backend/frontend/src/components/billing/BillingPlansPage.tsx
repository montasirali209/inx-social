...
function BillingSkeleton() {
  return <div aria-label="Loading billing details" className="grid animate-pulse gap-4 xl:grid-cols-12" role="status">
    <div className="h-72 rounded-panel border border-border-soft bg-panel/55 xl:col-span-8" />
    <div className="h-72 rounded-panel border border-border-soft bg-panel/55 xl:col-span-4" />
    <div className="h-48 rounded-panel border border-border-soft bg-panel/55 xl:col-span-7" />
    <div className="h-48 rounded-panel border border-border-soft bg-panel/55 xl:col-span-5" />
    <div className="h-[520px] rounded-panel border border-border-soft bg-panel/55 xl:col-span-12" />
  </div>
}
...
export function BillingPlansPage() {
  ...
  const rank = { trial: 0, creator: 1, pro: 2, business: 3, agency: 4 } as const
  const currentRank = rank[data.subscription.planId]
  const targetRank = rank[targetPlan]
  ...
  return <div className="space-y-4 pb-10">
    ...
    {visible('ai studio credits allowance trial creator pro business agency topup generation') && <AICreditAllowanceCard administrator={Boolean(data.subscription.administrator)} currentPlan={data.subscription.planId} onUpgrade={showPlans} />}
    {!data.subscription.administrator && visible('billing information payment method stripe security') && <BillingInformationCard onUpdate={() => void portal()} />}
    {!data.subscription.administrator && visible('change plan trial creator pro business agency upgrade downgrade compare pricing monthly yearly') && <section ref={planSection}><ChangePlanSection availability={data.billing.availability} current={data.subscription.planId} cycle={cycle} highlight={highlightPlans} onChoose={choosePlan} onCompare={() => setPanel('compare')} onCycle={setCycle} /></section>}
    <div className="grid items-stretch gap-4 xl:grid-cols-12">
      {!data.subscription.administrator && visible('invoices download pdf paid upcoming failed refunded') && <div className="xl:col-span-5"><InvoicesCard invoices={data.invoices} onViewAll={() => setPanel('invoices')} /></div>}
    ...
    {!data.subscription.administrator && visible('secure billing stripe payment privacy') && <SecureBillingFooter onLearn={() => setPanel('security')} />}

    <Modal footer={<><Button onClick={() => setPanel(null)}>Close</Button>{data.subscription.canManage && <Button onClick={() => void portal()} tone="primary"><ExternalLink className="size-4" />Open Stripe Customer Portal</Button>}</>} onClose={() => setPanel(null)} open={panel === 'subscription'} title="Manage Subscription"><div className="space-y-4"><div className="rounded-xl border border-border-soft bg-bg/35 p-4"><div className="flex items-center justify-between"><span className="text-sm text-text-muted">Current plan</span><strong>{currentPlan.name}</strong></div><div className="mt-3 flex items-center justify-between"><span className="text-sm text-text-muted">Billing cycle</span><strong className="capitalize">{data.subscription.legacyLifetime ? 'Legacy lifetime' : data.subscription.billingCycle}</strong></div><div className="mt-3 flex items-center justify-between"><span className="text-sm text-text-muted">Next payment / end date</span><strong>{data.subscription.renewalDate ? new Date(data.subscription.renewalDate).toLocaleDateString('en-GB') : 'None'}</strong></div></div><p className="text-sm leading-6 text-text-muted">Plan changes, cancellation, invoices and payment methods are managed securely by Stripe. Stripe shows any proration and effective date before you confirm.</p>{!data.subscription.canManage && <p className="rounded-xl border border-brand-amber/25 bg-brand-amber/8 p-3 text-sm text-[#fbbf24]">There is no paid Stripe subscription to manage. Choose Creator, Pro, Business or Agency below to start one.</p>}</div></Modal>
