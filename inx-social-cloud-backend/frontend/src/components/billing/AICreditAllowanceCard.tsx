...
export function AICreditAllowanceCard({ currentPlan, onUpgrade }: { currentPlan: PlanId; onUpgrade: () => void }) {
  ...
  if (access.isError || !access.data) {
    return <Card className="border-brand-amber/25 p-5"><div className="flex items-start gap-3"><Coins className="mt-0.5 size-5 text-brand-amber" /><div><h2 className="font-semibold">{plan.name} AI Studio Credits</h2><p className="mt-1 text-xs leading-5 text-text-muted">Your {plan.name} access includes {plan.monthlyAiCredits.toLocaleString()} shared AI credits{currentPlan === 'trial' ? ' for this trial' : ' per billing period'}. The live balance could not be loaded right now.</p><Button className="mt-3" onClick={() => void access.refetch()}>Refresh balance</Button></div></div></Card>
  }
  ...
  const topupsSupported = currentPlan !== 'trial' && Boolean(access.data.topupsEnabled && access.data.topupsSupported && packs.data?.supported && packs.data.packs.length)
  ...
  return <Card className="overflow-hidden border-brand-teal/25 bg-[radial-gradient(circle_at_92%_0%,rgba(34,211,238,.10),transparent_20rem),linear-gradient(145deg,rgba(6,37,45,.96),rgba(5,20,33,.98))] p-5 sm:p-6">
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(280px,.65fr)] lg:items-center">
      <div>
        <div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-2 text-sm font-semibold text-brand-cyan"><Sparkles className="size-4" />{plan.name} AI Studio Credits</span><span className="rounded-full border border-brand-green/25 bg-brand-green/10 px-2 py-1 text-[9px] font-bold uppercase tracking-[.14em] text-brand-green">Included</span></div>
