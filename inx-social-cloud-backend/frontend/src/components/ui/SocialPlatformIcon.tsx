...
const badgeStyles: Record<SocialPlatformName, string> = {
  ...
  x: 'border border-white/15 bg-[#090909] text-white',
...
function Glyph({ platform }: { platform: SocialPlatformName }) {
  if (platform === 'facebook') {
    return <svg aria-hidden="true" className="size-[78%] translate-y-[7%]" viewBox="0 0 24 24"><path d="M13.7 22v-8.1h2.72l.41-3.16H13.7V8.72c0-.91.25-1.54 1.57-1.54h1.68V4.36c-.29-.04-1.29-.13-2.45-.13-2.42 0-4.08 1.48-4.08 4.2v2.31H7.68v3.16h2.74V22h3.28Z" fill="currentColor" /></svg>
  }
  ...
  if (platform === 'youtube') {
    return <svg aria-hidden="true" className="size-[70%]" viewBox="0 0 24 24"><path d="M21.2 7.2a2.72 2.72 0 0 0-1.92-1.93C17.59 4.8 12 4.8 12 4.8s-5.59 0-7.28.47A2.72 2.72 0 0 0 2.8 7.2 28.4 28.4 0 0 0 2.33 12c0 1.62.15 3.22.47 4.8a2.72 2.72 0 0 0 1.92 1.93c1.69.47 7.28.47 7.28.47s5.59 0 7.28-.47a2.72 2.72 0 0 0 1.92-1.93c.32-1.58.47-3.18.47-4.8s-.15-3.22-.47-4.8Z" fill="white" /><path d="M10.06 15.08V8.92L15.4 12l-5.34 3.08Z" fill="#ff0000" /></svg>
  }
  if (platform === 'tiktok') {
  ...
  if (platform === 'whatsapp') {
    return <svg aria-hidden="true" className="size-[68%]" viewBox="0 0 24 24"><path d="M12.04 3.2a8.43 8.43 0 0 0-7.2 12.8L3.6 20.55l4.66-1.22a8.43 8.43 0 1 0 3.78-16.13Zm0 1.7a6.73 6.73 0 1 1-3.43 12.52l-.4-.24-2.3.6.62-2.24-.26-.42A6.73 6.73 0 0 1 12.04 4.9Zm-3.18 3.3c-.17 0-.45.06-.68.32-.24.26-.9.88-.9 2.14s.92 2.48 1.05 2.65c.13.17 1.8 2.75 4.38 3.85.61.26 1.09.42 1.46.54.61.19 1.17.17 1.61.1.49-.07 1.51-.62 1.72-1.21.21-.6.21-1.1.15-1.21-.06-.11-.23-.17-.49-.3-.26-.13-1.51-.75-1.75-.83-.23-.09-.4-.13-.58.13-.17.26-.66.83-.81 1-.15.17-.3.19-.55.06-.26-.13-1.08-.4-2.06-1.27-.76-.68-1.27-1.52-1.42-1.78-.15-.26-.02-.4.11-.53.12-.12.26-.3.39-.45.13-.15.17-.26.26-.43.09-.17.04-.32-.02-.45-.06-.13-.57-1.39-.79-1.9-.21-.5-.42-.43-.58-.44h-.5Z" fill="currentColor" /></svg>
  }
  if (platform === 'mastodon') {
    return <svg aria-hidden="true" className="size-[68%]" viewBox="0 0 24 24"><path d="M20.4 8.1c0-4.15-2.72-5.37-2.72-5.37C16.31 2.1 13.95 1.84 12 1.82h-.05c-1.95.02-4.3.28-5.68.91 0 0-2.72 1.22-2.72 5.37 0 .95-.02 2.08.01 3.28.11 4.06.74 8.05 4.5 9.05 1.73.46 3.21.56 4.4.49 2.16-.12 3.37-.77 3.37-.77l-.07-1.57s-1.54.49-3.27.43c-1.72-.06-3.54-.19-3.82-2.36a4.47 4.47 0 0 1-.04-.61s1.69.41 3.83.5c1.31.05 2.54-.08 3.79-.23 2.39-.29 4.47-1.78 4.73-3.15.41-2.16.38-5.27.38-5.27l-.96.17Zm-3.03 5.42h-2.22V8.08c0-1.15-.48-1.74-1.44-1.74-1.06 0-1.59.69-1.59 2.05v2.98H9.91V8.39c0-1.36-.53-2.05-1.59-2.05-.96 0-1.44.59-1.44 1.74v5.44H4.66V7.91c0-1.15.29-2.06.88-2.73.61-.67 1.4-1.01 2.39-1.01 1.14 0 2 .44 2.57 1.31l.55.92.55-.92c.57-.87 1.43-1.31 2.57-1.31.98 0 1.78.34 2.39 1.01.59.67.88 1.58.88 2.73v5.61h-.07Z" fill="currentColor" /></svg>
  }
...
export function SocialPlatformIcon({ platform, className = '' }: { platform: SocialPlatformName; className?: string }) {
  const defaultSize = /(?:^|\s)!?size-/.test(className) ? '' : 'size-6'
  return (
    <span
      aria-label={labels[platform]}
      className={`inline-grid ${defaultSize} shrink-0 place-items-center overflow-hidden !rounded-full shadow-[inset_0_1px_0_rgba(255,255,255,.2),0_4px_12px_rgba(0,0,0,.18)] ring-1 ring-white/10 ${badgeStyles[platform]} ${className}`}
      role="img"
