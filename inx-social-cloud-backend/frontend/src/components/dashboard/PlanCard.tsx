...
export function PlanCard({ overview }: { overview: StudioOverview | undefined }) {
  ...
  return (
    <section className="rounded-card border border-brand-blue/25 bg-gradient-to-br from-brand-blue/12 via-panel to-brand-purple/8 p-4 shadow-glow-blue">
      ...
      <a className="mt-4 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-lg border border-brand-blue/25 bg-brand-blue/8 px-3 text-xs font-semibold transition hover:bg-brand-blue/14 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-cyan" href="/portal/#overview"><Crown aria-hidden="true" className="size-4 text-brand-cyan" /> Manage plan</a>
    </section>
  )
}
