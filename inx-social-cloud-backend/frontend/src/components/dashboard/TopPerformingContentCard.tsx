...
export function TopPerformingContentCard({ items }: { items: TopContentItem[] }) {
  ...
  return (
    <ChartCard
      action={<a aria-label="Open Analytics" className="rounded-lg border border-border-soft px-2.5 py-1.5 text-[10px] text-text-muted transition hover:border-brand-cyan/45 hover:text-white focus-visible:outline-2 focus-visible:outline-brand-cyan" href="/app/analytics">Last 7 Days</a>}
    ...
    >
      ...
      </ol> : <div className="grid min-h-[225px] place-items-center px-6 text-center"><span><Trophy aria-hidden="true" className="mx-auto size-7 text-brand-cyan" /><strong className="mt-3 block text-sm">Performance ranking is waiting</strong><small className="mt-1 block max-w-xs leading-5 text-text-muted">Published content will be ranked when live engagement is available.</small><a className="mt-3 inline-flex items-center gap-1 text-xs text-brand-cyan" href="/app/analytics">Open Analytics <ArrowUpRight aria-hidden="true" className="size-3" /></a></span></div>}
    </ChartCard>
  )
}
