...
export function UploadProgressCard({ job }: { job: DashboardJob | null }) {
  return (
    <DashboardCard
      action={<a className="inline-flex items-center gap-1 text-xs font-semibold text-[#6db2ff] hover:text-brand-cyan focus-visible:outline-2 focus-visible:outline-brand-cyan" href="/app/bulk-scheduler">View queue <ArrowUpRight aria-hidden="true" className="size-3.5" /></a>}
      ...
      {!job ? (
        <div className="relative overflow-hidden px-4 py-3.5">
          ...
          <div className="relative flex items-center gap-4">
            ...
            <a aria-label="Upload a video" className="hidden size-10 shrink-0 place-items-center rounded-xl border border-brand-blue/25 bg-brand-blue/10 text-[#72b5ff] transition hover:bg-brand-blue/20 focus-visible:outline-2 focus-visible:outline-brand-cyan sm:grid" href="/app/bulk-scheduler"><Upload aria-hidden="true" className="size-4" /></a>
          </div>
