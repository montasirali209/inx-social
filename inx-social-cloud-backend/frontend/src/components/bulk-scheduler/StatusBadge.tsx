...
const styles: Record<UploadStatus, { label: string; className: string; icon: typeof CheckCircle2 }> = {
  ...
  checking: { label: 'Checking server', className: 'border-brand-cyan/30 bg-brand-cyan/10 text-brand-cyan', icon: LoaderCircle },
...
export function StatusBadge({ status }: { status: UploadStatus }) {
  ...
  return <span className={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 text-[11px] font-semibold ${definition.className}`}><Icon aria-hidden="true" className={`size-3.5 ${status === 'uploading' || status === 'checking' ? 'animate-spin motion-reduce:animate-none' : ''}`} />{definition.label}</span>
}
