    ...
    <>
      <div className="hidden overflow-hidden rounded-xl border border-border-soft lg:block">
        <table className="w-full border-collapse text-left text-xs">
          <thead className="bg-white/[0.035] text-[10px] uppercase tracking-[0.09em] text-text-soft"><tr><th className="px-3 py-2.5">Video</th><th className="px-3 py-2.5">Destination</th><th className="px-3 py-2.5">Status</th><th className="px-3 py-2.5">Result / Error</th></tr></thead>
          <tbody className="divide-y divide-white/6">
            {results.slice(0, 12).map((result) => <tr className="bg-black/8 transition hover:bg-brand-blue/5" key={result.id}><td className="px-3 py-2.5"><span className="flex min-w-0 items-center gap-2"><video aria-hidden="true" className="size-10 rounded-md bg-black object-cover" muted src={result.thumbnailUrl} /><strong className="max-w-48 truncate">{result.videoName}</strong></span></td><td className="px-3 py-2.5"><ResultDestinations destinations={destinations} ids={result.destinationIds} /></td><td className="px-3 py-2.5"><StatusBadge status={result.status} /></td><td className="max-w-56 px-3 py-2.5 text-text-muted"><span className="line-clamp-2">{result.errorMessage || result.resultId || (result.status === 'uploading' ? 'Meta is processing this video.' : 'Awaiting upload')}</span></td></tr>)}
          </tbody>
        </table>
      </div>
      <div className="grid gap-2 lg:hidden">{results.slice(0, 12).map((result) => <article className="rounded-xl border border-border-soft bg-black/12 p-3" key={result.id}><div className="flex items-start gap-3"><video aria-hidden="true" className="size-14 rounded-lg bg-black object-cover" muted src={result.thumbnailUrl} /><div className="min-w-0 flex-1"><strong className="block truncate text-sm">{result.videoName}</strong><div className="mt-2 flex flex-wrap items-center justify-between gap-2"><ResultDestinations destinations={destinations} ids={result.destinationIds} /><StatusBadge status={result.status} /></div></div></div>{(result.errorMessage || result.resultId) && <p className="mt-2 text-xs leading-5 text-text-muted">{result.errorMessage || result.resultId}</p>}</article>)}</div>
    </>
  )
}
