...
export function BulkSchedulerStats({ jobs, onOpen }: { jobs: DashboardJob[]; onOpen: (view: BulkHistoryView) => void }) {
  const scheduled = jobs.filter(job => job.status === 'QUEUED' || job.status === 'SCHEDULED').length
  const published = jobs.filter(job => job.status === 'PUBLISHED').length
  const processing = jobs.filter(job => ['AWAITING_UPLOAD', 'READY', 'PROCESSING'].includes(job.status)).length
  const needsReview = jobs.filter(job => job.status === 'FAILED' || job.status === 'CANCELLED').length
  const cards = [
    { label: 'All Bulk Jobs', value: jobs.length, detail: 'Publishing records', icon: ClipboardList, tone: 'border-brand-cyan/25 bg-brand-cyan/8 text-brand-cyan', view: 'all' as const },
    { label: 'Scheduled', value: scheduled, detail: 'Held in INX Social queue', icon: CalendarClock, tone: 'border-brand-blue/25 bg-brand-blue/8 text-brand-cyan', view: 'scheduled' as const },
