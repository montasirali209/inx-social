...
export function Topbar({ overview }: { overview?: StudioOverview }) {
  ...
  return (
    <header className="sticky top-0 z-20 flex min-h-[72px] items-center justify-between gap-4 border-b border-border-soft bg-bg/88 px-4 backdrop-blur-xl sm:px-6 xl:px-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <span aria-hidden="true" className="grid size-10 place-items-center rounded-full border border-brand-blue/35 bg-brand-blue/12 text-sm font-bold text-brand-cyan">{initials(name)}</span>
      </div>
    </header>
  )
}
