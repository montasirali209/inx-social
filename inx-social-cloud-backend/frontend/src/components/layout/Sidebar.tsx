...
const itemClasses = 'interactive-nav group relative flex min-h-10 items-center gap-3 overflow-hidden rounded-xl border px-3 text-sm font-medium focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-cyan lg:justify-center lg:px-2 xl:justify-start xl:px-3'
    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[264px] flex-col overflow-hidden border-r border-border-soft bg-[radial-gradient(circle_at_10%_8%,rgba(20,184,166,0.13),transparent_19rem),linear-gradient(180deg,rgba(3,17,30,0.99),rgba(2,12,22,0.99))] p-3 shadow-[18px_0_70px_rgba(0,0,0,0.3)] backdrop-blur-xl transition-transform duration-200 ease-out before:pointer-events-none before:absolute before:inset-0 before:bg-[linear-gradient(120deg,rgba(255,255,255,0.025),transparent_38%)] motion-reduce:transition-none lg:w-[88px] lg:translate-x-0 xl:w-[264px] ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <NavLink className="group mt-4 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-brand-cyan/45 bg-gradient-to-r from-brand-blue to-[#0f8f7f] px-4 text-sm font-bold text-white shadow-[0_14px_34px_rgba(20,184,166,0.2)] transition duration-200 hover:-translate-y-0.5 hover:brightness-110 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-cyan motion-reduce:transition-none" onClick={() => setOpen(false)} onFocus={() => warmRoute('/posts')} onPointerEnter={() => warmRoute('/posts')} onTouchStart={() => warmRoute('/posts')} to="/posts">
          ...
          <span className="lg:hidden xl:inline">Create New Post</span>
        </NavLink>
