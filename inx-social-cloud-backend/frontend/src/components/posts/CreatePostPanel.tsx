...
import { contentScore, improveCaption, postTypes } from '../../data/postsData'
import type { MediaItem, PostType } from '../../types/posts'
...
export function CreatePostPanel(props: Props) {
  ...
  return (
    <section className="interactive-surface rounded-panel border p-4 xl:p-5">
      ...
      <div className="mt-4"><div className="mb-2 flex items-center gap-2 text-[11px] font-semibold text-text-muted"><WandSparkles className="size-3.5 text-brand-cyan" />Content enhancement</div><div className="flex flex-wrap gap-2">{([['rewrite', 'Rewrite'], ['shorten', 'Shorten'], ['expand', 'Expand'], ['hashtags', 'Add Hashtags'], ['cta', 'Improve CTA']] as const).map(([action, label]) => <Button className="min-h-8 px-2.5 py-1 text-[10px]" disabled={!props.caption.trim()} key={action} onClick={() => props.setCaption(improveCaption(props.caption, action))} type="button" variant="ghost"><Sparkles className="size-3" />{label}</Button>)}<select aria-label="Caption tone" className="min-h-8 rounded-lg border border-border-soft bg-bg/50 px-2 text-[10px] text-text-muted outline-none focus:border-brand-cyan"><option>Professional</option><option>Friendly</option><option>Concise</option><option>Energetic</option></select></div></div>
      <div className="mt-4 grid gap-2 sm:grid-cols-2"><div className="rounded-xl border border-border-soft bg-bg/30 p-3"><span className="text-[10px] text-text-muted">Content Score</span><div className="mt-1 flex items-center gap-3"><strong className="text-lg text-brand-cyan">{score}/100</strong><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-white/5"><div className="h-full rounded-full bg-gradient-to-r from-brand-cyan to-brand-green transition-all" style={{ width: `${score}%` }} /></div></div></div><div className="rounded-xl border border-border-soft bg-bg/30 p-3"><span className="text-[10px] text-text-muted">Best Time To Post</span><strong className="mt-1 block text-xs text-text-main">Analytics recommendation</strong><p className="mt-1 text-[10px] text-text-soft">Choose a live destination to personalise timing.</p></div></div>
    </section>
  )
}
