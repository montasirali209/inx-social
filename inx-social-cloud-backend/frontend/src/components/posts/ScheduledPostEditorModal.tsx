...
import { cancelScheduledPost, replaceScheduledPostMedia, updateScheduledPost } from '../../lib/posts-api'
    ...
    <div className="fixed inset-0 z-[120] grid place-items-center overflow-y-auto bg-[#01070d]/88 p-4 backdrop-blur-md" onMouseDown={(event) => { if (event.currentTarget === event.target && !busy) onClose() }}>
      <section aria-modal="true" className="my-auto w-full max-w-2xl overflow-hidden rounded-panel border border-brand-cyan/30 bg-panel shadow-[0_35px_130px_rgba(0,0,0,.72)]" role="dialog">
        ...
        <div className="space-y-4 p-5">
          ...
          {job.contentType !== 'TEXT' && <label className="block rounded-xl border border-border-soft bg-bg/25 p-3">
        ...
        <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-border-soft bg-bg/20 p-4">
          <Button className="border-brand-red/25 text-brand-red hover:bg-brand-red/10" disabled={!editable || Boolean(busy)} onClick={() => void remove()} type="button" variant="ghost"><Trash2 className="size-4" />{busy === 'delete' ? 'Cancelling…' : 'Cancel schedule'}</Button>
          <div className="flex gap-2">
            <Button disabled={Boolean(busy)} onClick={onClose} type="button" variant="ghost">Close</Button>
            <Button disabled={!editable || Boolean(busy) || !date || !time} onClick={() => void save()} type="button" variant="primary">{busy === 'save' ? <RefreshCw className="size-4 animate-spin" /> : <Save className="size-4" />}{busy === 'save' ? 'Saving…' : 'Save changes'}</Button>
          </div>
        </footer>
      </section>
    </div>,
