...
const defaults: FormValues = {
  ...
  generateSlideCopy: true,
  generateVisuals: true,
}

const inputClass = 'mt-1.5 min-h-10 w-full rounded-xl border border-border-soft bg-bg/45 px-3 py-2 text-xs text-text-main outline-none transition placeholder:text-text-soft focus:border-brand-cyan focus:ring-2 focus:ring-brand-cyan/10'
const textareaClass = `${inputClass} min-h-28 resize-y leading-5`
...
function SelectField({ label, value, values, onChange }: { label: string; value: string; values: string[]; onChange: (value: string) => void }) {
  return <Field label={label}><select className={inputClass} onChange={(event) => onChange(event.target.value)} value={value}>{values.map((item) => <option key={item} value={item}>{item}</option>)}</select></Field>
}
...
function BrandKitField({ brandKits, value, onChange }: { brandKits: BrandKit[]; value: string; onChange: (value: string) => void }) {
  return <Field label="Brand kit"><select className={inputClass} onChange={(event) => onChange(event.target.value)} value={value}><option value="">Workspace defaults</option>{brandKits.map((kit) => <option key={kit.id} value={kit.id}>{kit.name}{kit.active ? ' · Active' : ''}</option>)}</select></Field>
}
...
export function GeneratedAssetPreview({ asset, onChange, onRegenerateSlide }: { asset: GeneratedAsset; onChange: (asset: GeneratedAsset) => void; onRegenerateSlide: (index: number) => void }) {
  return <div className="space-y-4">{asset.type === 'carousel' ? <CarouselEditor asset={asset} onChange={onChange} onRegenerateSlide={onRegenerateSlide} /> : asset.type === 'video' ? <VideoPreview asset={asset} /> : <div className="overflow-hidden rounded-2xl border border-border-soft bg-bg/40"><img alt={asset.altText || 'Generated social post'} className="mx-auto max-h-[560px] w-full object-contain" src={asset.url} /></div>}<div className="grid gap-3 lg:grid-cols-2"><Field label="Caption"><textarea className={`${textareaClass} min-h-36`} onChange={(event) => onChange({ ...asset, caption: event.target.value })} value={asset.caption || ''} /></Field><div className="space-y-3"><Field label="Hashtags"><input className={inputClass} onChange={(event) => onChange({ ...asset, hashtags: event.target.value.split(/\s+/).map((item) => item.replace(/^#/, '')).filter(Boolean) })} placeholder="#social #content" value={(asset.hashtags || []).map((tag) => `#${tag.replace(/^#/, '')}`).join(' ')} /></Field>{asset.type === 'image' && <Field label="Alt text"><textarea className={`${textareaClass} min-h-20`} onChange={(event) => onChange({ ...asset, altText: event.target.value })} value={asset.altText || ''} /></Field>}{asset.script !== undefined && <Field label="Script"><textarea className={`${textareaClass} min-h-24`} onChange={(event) => onChange({ ...asset, script: event.target.value })} value={asset.script || ''} /></Field>}</div></div></div>
}
...
function requestFor(type: AIContentType, values: FormValues, sourceAssetId?: string): GenerationRequest {
  ...
  return {
    ...
    options: {
      ...
      generateHashtags: values.generateHashtags,
      generateAltText: values.generateAltText,
      generateSlideCopy: values.generateSlideCopy,
      generateVisuals: values.generateVisuals,
    },
  }
}
...
function buildDraft(type: AIContentType, values: FormValues, asset: GeneratedAsset | null, existing: AIDraft | null | undefined, savedAssets: MediaAsset[]): AIDraft {
  return {
...
export function GenerationModal({ open, type, access, initialDraft, onClose, onSaved, onContinue, onToast }: { open: boolean; type: AIContentType | null; access: AIPlanAccess; initialDraft?: AIDraft | null; onClose: () => void; onSaved: (draft: AIDraft) => void; onContinue: (draft: AIDraft) => void; onToast: (message: string) => void }) {
  ...
  async function runGeneration(extraOptions: Record<string, unknown> = {}) {
    ...
    try {
      ...
      setMessage('Finalising content and generation metadata.')
      setAsset(result)
      setSavedAssets([])
      setStatus('completed')
  ...
  return createPortal(<div className="ai-studio-modal-backdrop fixed inset-0 z-[110] bg-[#01070d]/88 backdrop-blur-md" onMouseDown={(event) => { if (event.currentTarget === event.target && !generating) onClose() }}>
    <div className="flex h-dvh items-end justify-center p-0 sm:items-center sm:p-4" ref={containerRef}>
      <section aria-labelledby="ai-generation-title" aria-modal="true" className="ai-studio-modal-enter flex h-[100dvh] w-full max-w-[1500px] flex-col overflow-hidden border border-brand-cyan/25 bg-[linear-gradient(145deg,rgba(7,24,38,.99),rgba(3,13,24,.99))] shadow-[0_40px_150px_rgba(0,0,0,.75)] sm:h-[min(92dvh,940px)] sm:rounded-[26px]" role="dialog">
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-border-soft px-4 py-3 sm:px-5">
          <div className="flex min-w-0 items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl border border-brand-teal/25 bg-brand-teal/10 text-brand-cyan"><Icon className="size-4" /></span><div className="min-w-0"><span className="text-[8px] font-bold uppercase tracking-[.16em] text-text-soft">{definition.label}</span><h2 className="truncate text-base font-semibold" id="ai-generation-title">Create {definition.title}</h2></div></div>
          <div className="flex items-center gap-2"><span className="hidden rounded-full border border-brand-amber/20 bg-brand-amber/8 px-2.5 py-1 text-[9px] font-semibold text-brand-amber sm:inline-flex">⚡ ~{credits} credit{credits === 1 ? '' : 's'}</span><button aria-label={`Close ${definition.title} creator`} className="grid size-9 place-items-center rounded-xl border border-border-soft text-text-muted transition hover:border-brand-cyan/30 hover:text-white focus-visible:outline-2 focus-visible:outline-brand-cyan" disabled={generating} onClick={onClose} type="button"><X className="size-4" /></button></div>
        </header>
