...
function SourceAnalysisCard({ analysis }: { analysis: PostStudioSourceAnalysis }) {
  const facts = analysis.verifiedClaims.slice(0, 3)
  const angles = analysis.strongestAngles.slice(0, 2)
  const visuals = analysis.assetObservations.length ? analysis.assetObservations.slice(0, 2) : analysis.visualIdentity.slice(0, 2)
  return <div className="ml-9 animate-[fadeIn_.28s_ease-out] rounded-[20px] border border-brand-cyan/20 bg-[linear-gradient(145deg,rgba(5,31,45,.82),rgba(4,20,31,.88))] p-4 shadow-[0_14px_45px_rgba(6,182,212,.06)]">
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0"><span className="text-[8px] font-bold uppercase tracking-[.16em] text-brand-cyan">Source analysis complete</span><h4 className="mt-1 text-sm font-semibold text-white">{analysis.productName || 'Product and brand context understood'}</h4></div>
      <span className="shrink-0 rounded-full border border-brand-green/20 bg-brand-green/[.07] px-2 py-1 text-[8px] font-bold text-brand-green">Verified context</span>
    </div>
    {analysis.summary && <p className="mt-2 text-[10px] leading-5 text-text-muted">{analysis.summary}</p>}
    <div className="mt-3 grid gap-3 xl:grid-cols-3">
      {facts.length > 0 && <div><span className="text-[8px] font-bold uppercase tracking-[.12em] text-text-soft">Verified</span><ul className="mt-1.5 space-y-1 text-[9px] leading-4 text-text-muted">{facts.map((item) => <li key={item}>• {item}</li>)}</ul></div>}
      {visuals.length > 0 && <div><span className="text-[8px] font-bold uppercase tracking-[.12em] text-text-soft">Visual cues</span><ul className="mt-1.5 space-y-1 text-[9px] leading-4 text-text-muted">{visuals.map((item) => <li key={item}>• {item}</li>)}</ul></div>}
      {angles.length > 0 && <div><span className="text-[8px] font-bold uppercase tracking-[.12em] text-text-soft">Strong angles</span><ul className="mt-1.5 space-y-1 text-[9px] leading-4 text-text-muted">{angles.map((item) => <li key={item}>• {item}</li>)}</ul></div>}
    </div>
  </div>
}
...
function ReadyGenerateCard({
  ...
  return <div className="ml-9 animate-[fadeIn_.28s_ease-out] overflow-hidden rounded-[24px] border border-brand-cyan/35 bg-[linear-gradient(145deg,rgba(8,45,56,.94),rgba(4,24,37,.97))] shadow-[0_20px_70px_rgba(6,182,212,.12)]">
    <div className="border-b border-brand-cyan/15 px-4 py-4 sm:px-5">
      <div className="flex items-start gap-3">
        <span className="grid size-11 shrink-0 place-items-center rounded-2xl border border-brand-green/30 bg-brand-green/10 text-brand-green shadow-[0_0_28px_rgba(34,197,94,.12)]"><Check className="size-5" /></span>
        <div className="min-w-0">
          <span className="text-[9px] font-bold uppercase tracking-[.16em] text-brand-green">{regenerate ? 'Refinement complete' : 'Creative brief complete'}</span>
          <h3 className="mt-1 text-lg font-bold leading-6 text-white sm:text-xl">{regenerate ? 'Your updated post is ready' : 'Ready to generate your post'}</h3>
          <p className="mt-1.5 text-[10px] leading-5 text-text-muted">{regenerate ? 'Your latest chat changes have been applied to the creative brief. Generate the revised visual when you are happy with the direction.' : 'The concept, publishing copy and visual direction are prepared. Generate now or keep refining before spending credits.'}</p>
        </div>
      </div>
    ...
    <div className="grid gap-2 bg-black/10 p-3 sm:grid-cols-[1fr_auto_auto]">
      <button className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-brand-teal px-4 text-xs font-bold text-[#02130f] shadow-[0_0_28px_rgba(20,184,166,.2)] transition hover:-translate-y-0.5 hover:shadow-[0_0_38px_rgba(20,184,166,.3)] disabled:cursor-not-allowed disabled:opacity-45" disabled={disabled || insufficient} onClick={onGenerate} type="button"><WandSparkles className="size-4" />{regenerate ? 'Generate updated post · 5 credits' : 'Generate post now · 5 credits'}</button>
      <button className="min-h-11 rounded-xl border border-border-soft px-3 text-[9px] font-semibold text-text-muted transition hover:border-brand-cyan/25 hover:text-white" disabled={disabled} onClick={onRefine} type="button">Refine again</button>
...
function renderKey(brief: PostStudioBrief | null, platform: string, aspectRatio: PostStudioBrief['aspectRatio'], analysis: PostStudioSourceAnalysis | null) {
  if (!brief) return ''
...
export function ImagePostChatModal({
  ...
  async function askAssistant(nextMessages: PostStudioMessage[], nextUrls = urls, nextReferences = references) {
    ...
    try {
      ...
      const requestMessages = memory ? [memory, ...visible.slice(-17)] : visible.slice(-18)
      ...
      setSourceAnalysis(result.sourceAnalysis || sourceAnalysis)
  ...
  async function uploadReference(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const message: PostStudioMessage = { role: 'user', content: `I uploaded “${stored.fileName}” as a reference. Analyse the visible brand/product details and use it where it improves this post.` }
  ...
  async function generateImage() {
    ...
    try {
      const keyBeingRendered = currentRenderKey
      ...
      setAsset(generated)
      setLastRenderedKey(keyBeingRendered)
      onToast('Post creative generated successfully.')
    } catch (caught) {
  ...
  const sourceSummary = useMemo(() => [
    ...urls.map((url) => ({ key: url, label: (() => { try { return new URL(url).hostname } catch { return url } })(), icon: <Globe2 className="size-3" /> })),
  ...
  return createPortal(<div className="fixed inset-0 z-[110] bg-[#01070d]/90 backdrop-blur-md">
    <div className="flex h-dvh items-end justify-center p-0 sm:items-center sm:p-4">
      ...
      >
        ...
        <div className="relative z-10 grid min-h-0 flex-1 lg:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)]">
          <div className="flex min-h-0 flex-col border-b border-border-soft lg:border-b-0 lg:border-r">
            <div className="flex shrink-0 items-start justify-between gap-4 px-4 pb-3 pt-4 sm:px-5">
              <div><h3 className="text-sm font-semibold">Build the post with AI</h3><p className="mt-1 text-[9px] leading-4 text-text-soft">Share the idea. Add sources when useful. The Studio analyses product and brand evidence before it builds the final creative brief.</p></div>
            ...
            <div className="scrollbar-thin min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-2 sm:px-5">
              ...
              {thinking && <div className="flex items-center gap-2.5"><span className="grid size-7 place-items-center rounded-xl border border-brand-cyan/25 bg-brand-cyan/10 text-brand-cyan"><Bot className="size-3.5" /></span><div className="flex items-center gap-2 rounded-2xl border border-border-soft bg-white/[.035] px-3.5 py-3 text-[10px] text-text-muted"><span className="flex gap-1"><i className="size-1.5 animate-bounce rounded-full bg-brand-cyan [animation-delay:-.2s]" /><i className="size-1.5 animate-bounce rounded-full bg-brand-cyan [animation-delay:-.1s]" /><i className="size-1.5 animate-bounce rounded-full bg-brand-cyan" /></span>{sourceSummary.length ? 'Analysing sources and creative direction…' : 'Thinking about your post…'}</div></div>}
              {sourceAnalysis && !thinking && <SourceAnalysisCard analysis={sourceAnalysis} />}
              ...
              {renderNeeded && brief && !thinking && <ReadyGenerateCard aspectRatio={aspectRatio} brief={brief} disabled={busy} insufficient={insufficient} onAddReference={() => fileRef.current?.click()} onGenerate={() => void generateImage()} onRefine={() => inputRef.current?.focus()} platform={platform} regenerate={Boolean(asset)} sourceCount={sourceSummary.length} />}
            </div>

            <div className="shrink-0 border-t border-border-soft bg-bg/35 p-3 sm:p-4">
              {error && <div className="mb-2 rounded-2xl border border-brand-red/25 bg-brand-red/[.05] px-3.5 py-3"><div className="flex items-start gap-2.5"><span className="mt-0.5 grid size-7 shrink-0 place-items-center rounded-lg border border-brand-red/25 bg-brand-red/10 text-brand-red">!</span><div className="min-w-0"><strong className="block text-[10px] text-brand-red">The Studio could not complete that request</strong><p className="mt-1 text-[9px] leading-4 text-text-muted">{error}</p>{renderNeeded && <button className="mt-2 inline-flex items-center gap-1.5 text-[9px] font-semibold text-brand-cyan transition hover:text-white" disabled={busy || insufficient} onClick={() => void generateImage()} type="button"><WandSparkles className="size-3" />Retry render</button>}</div></div></div>}
          ...
          <div className="scrollbar-thin min-h-0 overflow-y-auto bg-black/[.07] p-4 sm:p-5">
            {!asset ? <div className={`relative grid min-h-[520px] h-full place-items-center overflow-hidden rounded-[28px] border bg-[radial-gradient(circle_at_50%_45%,rgba(34,211,238,.075),transparent_23rem)] p-7 text-center ${renderNeeded ? 'border-brand-green/25' : 'border-dashed border-brand-cyan/18'}`}>
              ...
              <div className="relative max-w-lg">
                <span className={`relative mx-auto grid size-16 place-items-center rounded-[22px] border shadow-[0_0_50px_rgba(34,211,238,.08)] ${renderNeeded ? 'border-brand-green/30 bg-brand-green/[.08] text-brand-green' : 'border-brand-cyan/25 bg-brand-cyan/[.07] text-brand-cyan'}`}>{renderNeeded ? <Check className="size-7" /> : <Sparkles className="size-7" />}<span className="absolute inset-0 animate-ping rounded-[22px] border border-brand-cyan/10 [animation-duration:3s]" /></span>
                {generating ? <><h3 className="mt-5 text-xl font-semibold">Creating your post…</h3><div className="mx-auto mt-5 h-1.5 max-w-xs overflow-hidden rounded-full bg-white/[.05]"><div className="h-full w-2/3 animate-pulse rounded-full bg-gradient-to-r from-brand-teal to-brand-cyan" /></div><div className="mt-4 flex justify-center gap-2 text-[9px] text-text-soft"><span>Creative direction</span><span>•</span><span>References</span><span>•</span><span>Final layout</span></div></> : renderNeeded && brief ? <><span className="mt-5 inline-flex rounded-full border border-brand-green/25 bg-brand-green/10 px-2.5 py-1 text-[9px] font-bold uppercase tracking-[.14em] text-brand-green">Ready</span><h3 className="mt-3 text-xl font-bold">Your post is ready to generate.</h3><p className="mx-auto mt-2 max-w-md text-xs leading-5 text-text-muted">{brief.headline ? `Creative direction prepared for “${brief.headline}”.` : 'Creative direction, caption and publishing copy are prepared.'}</p><div className="mt-4 flex flex-wrap justify-center gap-2"><AnalysisPill icon={<MessageSquareText className="size-3" />}>{platform}</AnalysisPill><AnalysisPill icon={<ImagePlus className="size-3" />}>{aspectRatio}</AnalysisPill>{sourceSummary.length > 0 && <AnalysisPill icon={<Globe2 className="size-3" />}>{sourceSummary.length} source{sourceSummary.length === 1 ? '' : 's'}</AnalysisPill>}</div><Button className="mt-6 min-h-12 px-6 text-xs font-bold" disabled={insufficient || busy} onClick={() => void generateImage()} variant="primary"><WandSparkles className="size-4" />Generate final post · 5 credits</Button>{insufficient && <p className="mt-2 text-[9px] text-brand-red">You need 5 AI credits for the final render.</p>}</> : <><h3 className="mt-5 text-lg font-semibold">Your post comes together here.</h3><p className="mx-auto mt-2 max-w-md text-xs leading-5 text-text-muted">Chat naturally on the left. Add URLs or references when they matter; the Studio will analyse them before preparing the creative.</p><div className="mt-5 flex flex-wrap justify-center gap-2"><AnalysisPill icon={<MessageSquareText className="size-3" />}>Ask & refine</AnalysisPill><AnalysisPill icon={<Globe2 className="size-3" />}>Analyse URLs</AnalysisPill><AnalysisPill icon={<ImagePlus className="size-3" />}>Use references</AnalysisPill></div></>}
              </div>
            </div> : <div>
              <div className="mb-4 flex flex-wrap items-start justify-between gap-3"><div><span className="inline-flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-[.15em] text-brand-green"><Check className="size-3" />Generated</span><h3 className="mt-1 text-sm font-semibold">Review & refine your post</h3><p className="mt-1 text-[9px] text-text-soft">Keep chatting on the left. If you change the creative, the updated Generate button will appear in the conversation.</p></div><span className="rounded-full border border-brand-green/25 bg-brand-green/10 px-2.5 py-1 text-[9px] font-semibold text-brand-green">5 credits used</span></div>
              {renderNeeded && <div className="mb-3 rounded-2xl border border-brand-cyan/25 bg-brand-cyan/[.045] px-3.5 py-3 text-[10px] text-text-muted"><strong className="text-brand-cyan">Updated brief ready.</strong> Generate the revised visual from the chat when you are happy with the refinement.</div>}
        ...
        <footer className="relative z-10 flex shrink-0 items-center justify-between gap-3 border-t border-border-soft bg-bg/45 px-4 py-3 sm:px-5"><span className="text-[8px] leading-4 text-text-soft">Studio conversation and source analysis are included. Credits are used only when a final image render completes.</span><div className="flex items-center gap-2">{generating && <Button onClick={() => controllerRef.current?.abort()}><X className="size-3.5" />Cancel</Button>}<Button disabled={busy} onClick={onClose}>Close</Button></div></footer>
      </section>
    </div>
  </div>, document.body)
}
