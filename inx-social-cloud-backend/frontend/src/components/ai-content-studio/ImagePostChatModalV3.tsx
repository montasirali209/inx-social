...
function renderKey(brief: PostStudioBrief | null, platform: string, ratio: PostStudioBrief['aspectRatio'], analysis: PostStudioSourceAnalysis | null) {
  if (!brief) return ''
  return JSON.stringify({ objective: brief.objective, audience: brief.audience, platform, ratio, tone: brief.tone, visualStyle: brief.visualStyle, headline: brief.headline, supportingCopy: brief.supportingCopy, cta: brief.cta, visualDirection: brief.visualDirection, analysis: analysis?.fingerprint || '' })
}
...
function buildDraft(asset: GeneratedAsset, messages: PostStudioMessage[], brief: PostStudioBrief | null, existing?: AIDraft | null): AIDraft {
  ...
  return {
    id: existing?.id || crypto.randomUUID(), contentType: 'image_post', title: (brief?.headline || prompt || 'AI image post').slice(0, 70), thumbnailUrl: asset.thumbnailUrl || asset.url,
    updatedAt: new Date().toISOString(), status: 'ready', prompt: prompt.slice(0, 1500), caption: brief?.caption || asset.caption || '', hashtags: brief?.hashtags || asset.hashtags || [], altText: brief?.altText || asset.altText || '',
    asset: { ...asset, caption: brief?.caption || asset.caption || '', hashtags: brief?.hashtags || asset.hashtags || [], altText: brief?.altText || asset.altText || '' },
...
function Bubble({ message }: { message: PostStudioMessage }) {
  ...
  return <div className={`flex gap-2.5 ${assistant ? 'justify-start' : 'justify-end'}`}>
    ...
    <div className={`max-w-[88%] rounded-2xl border px-3.5 py-3 text-[11px] leading-5 shadow-[0_10px_30px_rgba(0,0,0,.16)] ${assistant ? 'border-border-soft bg-white/[.035] text-text-main' : 'border-brand-teal/30 bg-brand-teal/12 text-white'}`}><p className="whitespace-pre-wrap">{message.content}</p></div>
  </div>
}
...
function SourceCard({ analysis }: { analysis: PostStudioSourceAnalysis }) {
  return <div className="ml-9 rounded-[20px] border border-brand-cyan/20 bg-[linear-gradient(145deg,rgba(5,31,45,.82),rgba(4,20,31,.88))] p-4">
    <div className="flex items-start justify-between gap-3"><div><span className="text-[8px] font-bold uppercase tracking-[.16em] text-brand-cyan">Source analysis complete</span><h4 className="mt-1 text-sm font-semibold">{analysis.productName || 'Product and brand context understood'}</h4></div><span className="rounded-full border border-brand-green/20 bg-brand-green/[.07] px-2 py-1 text-[8px] font-bold text-brand-green">Verified context</span></div>
...
function RecoveryChoice({ recovery, onResume, onFresh }: { recovery: PostStudioRecovery; onResume: () => void; onFresh: () => void }) {
  ...
  return <div className="absolute inset-0 z-40 grid place-items-center bg-[#01070d]/88 p-5 backdrop-blur-md">
    <div className="w-full max-w-xl overflow-hidden rounded-[28px] border border-brand-cyan/30 bg-[linear-gradient(145deg,#071d2a,#04111c)] shadow-[0_36px_120px_rgba(0,0,0,.72)]">
      <div className="p-6 sm:p-7"><span className="grid size-12 place-items-center rounded-2xl border border-brand-cyan/25 bg-brand-cyan/10 text-brand-cyan"><Clock3 className="size-5" /></span><span className="mt-5 block text-[9px] font-bold uppercase tracking-[.17em] text-brand-cyan">Unfinished work found</span><h3 className="mt-1 text-xl font-bold">Continue your previous AI post?</h3><p className="mt-2 text-xs leading-5 text-text-muted">Your last Image Post session was saved automatically before the workspace session ended. Resume exactly where you left off, or start a clean post.</p>
...
export function ImagePostChatModal({ open, type, access, initialDraft, onClose, onSaved, onContinue, onToast }: { open: boolean; type: AIContentType | null; access: AIPlanAccess; initialDraft?: AIDraft | null; onClose: () => void; onSaved: (draft: AIDraft) => void; onContinue: (draft: AIDraft) => void; onToast: (message: string) => void }) {
  ...
  function resumeRecovery() {
    ...
    setRecoveryCandidate(null); setRecoveryResolved(true)
    onToast('Previous AI Post Studio session restored.')
  ...
  async function generateImage() {
    ...
    } catch (caught) { if (!(caught instanceof DOMException && caught.name === 'AbortError')) setError(caught instanceof Error ? caught.message : 'The post image could not be generated.') } finally { controllerRef.current = null; setGenerating(false) }
  }
  ...
  const sources = useMemo(() => [...urls.map((url) => ({ id: url, label: (() => { try { return new URL(url).hostname } catch { return url } })(), icon: <Globe2 className="size-3" /> })), ...references.map((item) => ({ id: item.id, label: item.fileName, icon: <FileImage className="size-3" /> }))], [urls, references])
  ...
  return createPortal(<div className="fixed inset-0 z-[110] bg-[#01070d]/90 backdrop-blur-md"><div className="flex h-dvh items-end justify-center p-0 sm:items-center sm:p-4"><section aria-modal="true" className="relative flex h-[100dvh] w-full max-w-[1560px] flex-col overflow-hidden border border-brand-cyan/25 bg-[linear-gradient(145deg,rgba(5,22,34,.995),rgba(2,12,22,.995))] shadow-[0_40px_160px_rgba(0,0,0,.78)] sm:h-[min(94dvh,960px)] sm:rounded-[28px]" role="dialog">
    ...
    <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)]">
      <div className="flex min-h-0 flex-col border-r border-border-soft"><div className="shrink-0 px-5 pb-3 pt-4"><h3 className="text-sm font-semibold">Build the post with AI</h3><p className="mt-1 text-[9px] text-text-soft">Share the idea. Add sources when useful. Unfinished work is saved automatically for recovery.</p></div>{sources.length > 0 && <div className="flex flex-wrap gap-1.5 px-5 pb-3">{sources.map((source) => <Pill icon={source.icon} key={source.id}>{source.label}</Pill>)}</div>}
        <div className="scrollbar-thin min-h-0 flex-1 space-y-3 overflow-y-auto px-5 py-2">{messages.map((message, index) => <Bubble key={`${message.role}-${index}-${message.content.slice(0, 16)}`} message={message} />)}{thinking && <div className="flex items-center gap-2.5"><span className="grid size-7 place-items-center rounded-xl border border-brand-cyan/25 bg-brand-cyan/10 text-brand-cyan"><Bot className="size-3.5" /></span><div className="rounded-2xl border border-border-soft bg-white/[.035] px-3.5 py-3 text-[10px] text-text-muted"><LoaderCircle className="mr-2 inline size-3 animate-spin text-brand-cyan" />{sources.length ? 'Analysing sources and creative direction…' : 'Thinking about your post…'}</div></div>}{sourceAnalysis && !thinking && <SourceCard analysis={sourceAnalysis} />}{assistantResult?.quickReplies?.length && !renderNeeded && !asset ? <div className="flex flex-wrap gap-1.5 pl-9">{assistantResult.quickReplies.map((reply) => <button className="rounded-full border border-brand-cyan/20 bg-brand-cyan/[.05] px-3 py-1.5 text-[9px] font-semibold text-brand-cyan" disabled={busy} key={reply} onClick={() => void sendText(reply)} type="button">{reply}</button>)}</div> : null}{renderNeeded && brief && !thinking && <div className="ml-9 overflow-hidden rounded-[24px] border border-brand-cyan/35 bg-[linear-gradient(145deg,rgba(8,45,56,.94),rgba(4,24,37,.97))]"><div className="p-5"><span className="text-[9px] font-bold uppercase tracking-[.16em] text-brand-green">{asset ? 'Refinement complete' : 'Creative brief complete'}</span><h3 className="mt-1 text-xl font-bold">{asset ? 'Your updated post is ready' : 'Ready to generate your post'}</h3><p className="mt-1.5 text-[10px] leading-5 text-text-muted">{asset ? 'Your latest chat changes are applied. Generate the revised visual when ready.' : 'The concept, copy and visual direction are ready.'}</p><div className="mt-3 flex flex-wrap gap-2"><Pill icon={<MessageSquareText className="size-3" />}>{platform}</Pill><Pill icon={<Sparkles className="size-3" />}>{aspectRatio}</Pill></div></div><div className="grid gap-2 border-t border-brand-cyan/15 p-3 sm:grid-cols-[1fr_auto_auto]"><Button disabled={busy || insufficient} onClick={() => void generateImage()} variant="primary"><WandSparkles className="size-4" />{asset ? 'Generate updated post · 5 credits' : 'Generate post now · 5 credits'}</Button><Button disabled={busy} onClick={() => inputRef.current?.focus()}>Refine again</Button><Button disabled={busy} onClick={() => fileRef.current?.click()}>Add reference</Button></div></div>}</div>
        <div className="shrink-0 border-t border-border-soft bg-bg/35 p-4">{error && <div className="mb-2 rounded-xl border border-brand-red/25 bg-brand-red/[.05] px-3 py-2 text-[9px] text-brand-red">{error}</div>}{uploading && <div className="mb-2 text-[9px] text-text-muted">Uploading reference · {uploadProgress}%</div>}<div className="rounded-2xl border border-border-strong bg-[#04111c]/85 p-2"><textarea aria-label="Message AI Post Studio" className="min-h-20 w-full resize-none bg-transparent px-2 py-1.5 text-xs text-text-main outline-none" disabled={busy} maxLength={4000} onChange={(event) => setComposer(event.target.value)} onKeyDown={handleKey} placeholder="Describe the post, paste a product URL, or tell the Studio what to change…" ref={inputRef} value={composer} /><div className="flex items-center justify-between border-t border-border-soft pt-2"><div className="flex items-center gap-1"><input accept="image/png,image/jpeg,image/webp,image/gif" className="sr-only" onChange={(event) => void uploadReference(event)} ref={fileRef} type="file" /><button className="grid size-8 place-items-center rounded-lg text-text-muted hover:text-brand-cyan" disabled={busy || references.length >= 4} onClick={() => fileRef.current?.click()} type="button"><Paperclip className="size-3.5" /></button><button className="inline-flex min-h-8 items-center gap-1.5 rounded-lg px-2 text-[9px] text-text-muted hover:text-brand-cyan" disabled={busy || urls.length >= 2} onClick={() => { setComposer((value) => value || 'https://'); inputRef.current?.focus() }} type="button"><Globe2 className="size-3.5" />Add URL</button></div><button className="grid size-9 place-items-center rounded-xl bg-brand-teal text-[#02130f] disabled:opacity-40" disabled={busy || !composer.trim()} onClick={() => void sendText()} type="button"><Send className="size-4" /></button></div></div><details className="mt-2 rounded-xl border border-border-soft"><summary className="cursor-pointer px-3 py-2 text-[9px] text-text-soft">Optional controls · AI chooses sensible defaults</summary><div className="grid gap-2 border-t border-border-soft p-3 sm:grid-cols-2"><select className="min-h-9 rounded-xl border border-border-soft bg-bg/50 px-2.5 text-[10px]" onChange={(event) => setPlatform(event.target.value)} value={platform}>{PLATFORMS.map((item) => <option key={item}>{item}</option>)}</select><select className="min-h-9 rounded-xl border border-border-soft bg-bg/50 px-2.5 text-[10px]" onChange={(event) => setAspectRatio(event.target.value as PostStudioBrief['aspectRatio'])} value={aspectRatio}>{ASPECTS.map((item) => <option key={item}>{item}</option>)}</select></div></details></div>
      </div>
      <div className="scrollbar-thin min-h-0 overflow-y-auto bg-black/[.07] p-5">{!asset ? <div className={`grid min-h-[520px] h-full place-items-center rounded-[28px] border p-7 text-center ${renderNeeded ? 'border-brand-green/25' : 'border-dashed border-brand-cyan/18'}`}><div className="max-w-lg"><span className="mx-auto grid size-16 place-items-center rounded-[22px] border border-brand-cyan/25 bg-brand-cyan/[.07] text-brand-cyan">{generating ? <LoaderCircle className="size-7 animate-spin" /> : renderNeeded ? <Check className="size-7 text-brand-green" /> : <Sparkles className="size-7" />}</span><h3 className="mt-5 text-lg font-semibold">{generating ? 'Creating your final post…' : renderNeeded ? 'Your post is ready to generate.' : 'Your post comes together here.'}</h3><p className="mx-auto mt-2 max-w-md text-xs leading-5 text-text-muted">{generating ? 'Applying your approved creative direction and references.' : renderNeeded ? 'Use the large Generate action in the chat to create the final visual.' : 'Chat naturally on the left. The Studio analyses URLs and references before preparing the final post.'}</p></div></div> : <div><div className="mb-4"><span className="text-[9px] font-bold uppercase tracking-[.15em] text-brand-green">✓ Generated</span><h3 className="mt-1 text-sm font-semibold">Review & refine your post</h3><p className="mt-1 text-[9px] text-text-soft">Keep chatting on the left. If you change the creative direction, a new Generate updated post button appears in chat.</p></div><div className="overflow-hidden rounded-3xl border border-border-soft bg-black/25"><img alt={brief?.altText || 'Generated social post'} className="mx-auto max-h-[590px] w-full object-contain" src={asset.url} /></div><div className="mt-4 grid gap-3 xl:grid-cols-[1.35fr_.65fr]"><div className="rounded-2xl border border-border-soft bg-bg/35 p-4"><span className="text-[9px] font-bold uppercase tracking-[.13em] text-text-soft">Caption</span><p className="mt-2 whitespace-pre-wrap text-[11px] leading-5">{caption || 'No caption generated yet.'}</p></div><div className="space-y-3"><div className="rounded-2xl border border-border-soft bg-bg/35 p-4"><span className="text-[9px] font-bold uppercase tracking-[.13em] text-text-soft">Hashtags</span><p className="mt-2 text-[10px] leading-5 text-text-muted">{hashtags.length ? hashtags.map((tag) => `#${tag.replace(/^#/, '')}`).join(' ') : 'No hashtags suggested.'}</p></div>{brief?.cta && <div className="rounded-2xl border border-brand-cyan/15 bg-brand-cyan/[.035] p-4"><span className="text-[9px] font-bold uppercase tracking-[.13em] text-brand-cyan">CTA</span><p className="mt-2 text-[11px] font-semibold">{brief.cta}</p></div>}</div></div><div className="mt-4 grid gap-2 sm:grid-cols-3"><Button onClick={() => inputRef.current?.focus()}><MessageSquareText className="size-3.5" />Refine in chat</Button><Button onClick={() => void saveDraft()}><Save className="size-3.5" />Save draft</Button><Button onClick={continueToPosts} variant="primary">Continue to Posts <ArrowRight className="size-3.5" /></Button></div></div>}</div>
    </div>
