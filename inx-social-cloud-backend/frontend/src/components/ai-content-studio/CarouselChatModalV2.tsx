...
function buildDraft(asset: GeneratedAsset, brief: PostStudioBrief | null, messages: PostStudioMessage[], existing?: AIDraft | null): AIDraft {
  ...
  return {
    ...
    title: (brief?.headline || prompt).slice(0, 70),
    thumbnailUrl: asset.thumbnailUrl || asset.slides?.[0]?.thumbnailUrl || asset.url,
...
export function CarouselChatModal({ open, type, access, initialDraft, onClose, onSaved, onContinue, onToast }: {
  ...
  async function generate() {
    ...
    try {
      ...
      setAsset(result)
      setActiveSlide(0)
      setLastRenderedKey(currentKey)
      setRefineMode(false)
      onToast(wasRefinement ? `Carousel refinement applied · ${result.creditsUsed} credits used.` : `${slides}-slide carousel created · ${result.creditsUsed} credits used.`)
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Carousel generation failed.')
    } finally {
      setGenerating(false)
    }
  }
  ...
  const slideOptions = [3, 4, 5, 6, 7, 8, 9, 10].map((value) => ({
    value,
    label: `${value} slides`,
    meta: value <= 5 ? 'Concise story' : value <= 8 ? 'Detailed carousel' : 'Long-form carousel',
    icon: <Layers3 className="size-3.5" />,
  }))
  const platformOptions = PLATFORMS.map((value) => ({
    value,
    label: value,
    meta: value === 'LinkedIn' ? 'Professional carousel' : 'Social feed carousel',
    icon: <Sparkles className="size-3.5" />,
  }))
  const ratioOptions = RATIOS.map((value) => ({
    value,
    label: value,
    meta: value === '4:5' ? 'Portrait feed · recommended' : 'Square feed',
      ...
      <div className="flex h-[min(900px,95vh)] w-full max-w-[1540px] flex-col overflow-hidden rounded-[30px] border border-brand-cyan/25 bg-[radial-gradient(circle_at_12%_15%,rgba(0,214,192,.09),transparent_28%),linear-gradient(145deg,#061824,#020b13)] shadow-[0_44px_160px_rgba(0,0,0,.72)]">
        <header className="flex min-h-16 items-center justify-between border-b border-border-soft px-4 sm:px-6">
          <div>
            ...
            <h2 className="mt-1 text-base font-bold">AI Post Studio <span className="font-medium text-text-soft">· Carousel</span></h2>
          </div>
        ...
        <div className="grid min-h-0 flex-1 lg:grid-cols-[47%_53%]">
          <section className="flex min-h-0 flex-col border-r border-border-soft">
            <div className="border-b border-border-soft px-4 py-3 sm:px-5">
              <h3 className="text-sm font-bold">Build the carousel with AI</h3>
              <p className="mt-1 text-[9px] text-text-soft">Research, references, narrative planning and post-generation refinements happen here.</p>
            </div>
            ...
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4 sm:px-5">
              ...
              {sourceAnalysis && (
                <div className="ml-9 rounded-2xl border border-brand-cyan/20 bg-brand-cyan/[.035] p-4">
                  <span className="text-[8px] font-bold uppercase tracking-[.15em] text-brand-cyan">Source analysis complete</span>
                  <p className="mt-2 text-[10px] leading-5 text-text-muted">{sourceAnalysis.summary}</p>
                  {sourceAnalysis.strongestAngles.length > 0 && <p className="mt-2 text-[9px] text-text-soft">Strong angle: {sourceAnalysis.strongestAngles[0]}</p>}
                </div>
              )}
            ...
            <div className="space-y-2 border-t border-border-soft p-3 sm:p-4">
                ...
                <div className={`rounded-2xl border px-3 py-2.5 transition ${refineMode ? 'border-brand-cyan/45 bg-brand-cyan/[.06]' : 'border-border-soft bg-black/10'}`}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="mt-0.5 text-[9px] text-text-soft">Tell AI what to change. Your generated version remains safe until you apply the new render.</p>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      <button className="rounded-lg border border-border-soft px-2 py-1.5 text-[9px] text-text-muted hover:border-brand-cyan/30 hover:text-brand-cyan" onClick={() => startRefinement(`On slide ${activeSlide + 1}, `)} type="button">Current slide</button>
                      <button className="rounded-lg border border-border-soft px-2 py-1.5 text-[9px] text-text-muted hover:border-brand-cyan/30 hover:text-brand-cyan" onClick={() => startRefinement('Make the headlines shorter and more impactful while keeping the same message.')} type="button">Shorter copy</button>
                      <button className="rounded-lg border border-border-soft px-2 py-1.5 text-[9px] text-text-muted hover:border-brand-cyan/30 hover:text-brand-cyan" onClick={() => startRefinement('Make the carousel feel more premium and polished while keeping the existing brand direction.')} type="button">More premium</button>
                      <button className="rounded-lg border border-border-soft px-2 py-1.5 text-[9px] text-text-muted hover:border-brand-cyan/30 hover:text-brand-cyan" onClick={() => startRefinement('Strengthen the final call to action without changing the core story.')} type="button">Improve CTA</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              <details className="rounded-2xl border border-border-soft bg-black/10">
                ...
                <div className="grid gap-3 border-t border-border-soft p-3 sm:grid-cols-3">
                  ...
                  <StudioSelect label="Platform" value={platform} options={platformOptions} onChange={setPlatform} accent="green" />
                  <StudioSelect label="Aspect ratio" value={aspectRatio} options={ratioOptions} onChange={setAspectRatio} accent="violet" />
                </div>
              </details>
            </div>
          </section>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto rounded-[24px] border border-border-soft bg-[radial-gradient(circle_at_50%_30%,rgba(0,214,192,.08),transparent_45%)] p-4">
              {!asset && !generating && (
                <div className="grid h-full min-h-[420px] place-items-center text-center">
                  <div>
                    <span className="mx-auto grid size-14 place-items-center rounded-2xl border border-brand-cyan/25 bg-brand-cyan/10 text-brand-cyan"><Sparkles className="size-6" /></span>
                    <h3 className="mt-4 text-lg font-bold">Your carousel comes together here.</h3>
                    <p className="mx-auto mt-2 max-w-md text-[10px] leading-5 text-text-muted">Plan the story in chat, review the research, then generate a coordinated sequence rather than unrelated images.</p>
                  </div>
                </div>
              )}
              ...
              {asset && selectedSlide && (
                <div>
                  ...
                  <div className="mt-4 rounded-2xl border border-brand-cyan/20 bg-brand-cyan/[.035] p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2"><WandSparkles className="size-3.5 text-brand-cyan" /><strong className="text-[10px]">Need to change something?</strong></div>
                        <p className="mt-1 text-[9px] leading-4 text-text-soft">Refine the whole carousel or target the currently selected slide in chat. Nothing replaces this version until you approve and render the changes.</p>
                      </div>
                      <Button size="sm" onClick={() => startRefinement(`On slide ${activeSlide + 1}, `)}>Refine slide {activeSlide + 1}</Button>
                    </div>
                  </div>
                </div>
              )}
