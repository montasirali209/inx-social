...
import { ArrowRight, Bot, Check, Film, ImagePlus, LoaderCircle, Music2, Save, Sparkles, Upload, WandSparkles, X, Zap } from 'lucide-react'
...
const ASPECTS: VideoStudioSelection['aspectRatio'][] = ['9:16', '16:9', '1:1', '4:5']
...
function buildDraft(asset: GeneratedAsset, prompt: string, brief: PostStudioBrief | null, existing?: AIDraft | null): AIDraft {
  return {
    id: existing?.id || crypto.randomUUID(), contentType: 'short_video', title: (brief?.headline || prompt || 'AI video').slice(0, 70), thumbnailUrl: asset.thumbnailUrl || asset.url,
    updatedAt: new Date().toISOString(), status: 'ready', prompt: prompt.slice(0, 1500), caption: brief?.caption || asset.caption || '', hashtags: brief?.hashtags || asset.hashtags || [], altText: '',
    asset: { ...asset, caption: brief?.caption || asset.caption || '', hashtags: brief?.hashtags || asset.hashtags || [] }, mediaLibraryAsset: existing?.mediaLibraryAsset || null, mediaLibraryAssets: existing?.mediaLibraryAssets || [],
  }
}
...
export function VideoStudioModal({ open, type, access, initialDraft, onClose, onSaved, onContinue, onToast }: { open: boolean; type: AIContentType | null; access: AIPlanAccess; initialDraft?: AIDraft | null; onClose: () => void; onSaved: (draft: AIDraft) => void; onContinue: (draft: AIDraft) => void; onToast: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    void getVideoModels().then(value => setModels(value)).catch(caught => setError(caught instanceof Error ? caught.message : 'Video models could not be loaded.')).finally(() => setLoadingModels(false))
  }, [open, type, initialDraft])

  useEffect(() => {
    if (!selected) return
    if (!selected.durations.includes(duration)) setDuration(selected.durations[0])
    if (!selected.resolutions.includes(resolution)) setResolution(selected.resolutions.includes('720p') ? '720p' : selected.resolutions[0])
    if (!selected.draftSupported && draft) setDraft(false)
  }, [selected, duration, resolution, draft])

  useEffect(() => {
    if (!open || !selected) return
    setEstimating(true)
    const timer = window.setTimeout(() => {
      void estimateVideoCredits(selection).then(value => setCredits(value.credits)).catch(caught => setError(caught instanceof Error ? caught.message : 'Video credits could not be estimated.')).finally(() => setEstimating(false))
    }, 180)
  ...
  async function polishWithAI() {
    ...
    try {
      ...
      const result = await sendPostStudioMessage({
        messages: [{ role: 'user', content: `Create a production-ready short-form video/Reel creative brief from this idea. Focus on motion, camera, scene progression, hook and social pacing rather than a static image. ${sourceInstruction}\n\n${prompt}` }],
        urls: extractUrls(prompt), referenceAssetIds: sourceAsset ? [sourceAsset.id] : [], platform: aspectRatio === '9:16' ? 'Instagram Reels / TikTok' : 'Social video', aspectRatio,
      })
      setBrief(result.brief)
      if (result.brief.visualDirection) setPrompt(result.brief.visualDirection.slice(0, 1500))
      onToast(result.routing.deepAnalysisPerformed ? 'Website/reference analysed and video brief prepared.' : 'Video brief polished with AI.')
  ...
  async function generate() {
    if (prompt.trim().length < 2 || generating || insufficient) return
    const controller = new AbortController(); controllerRef.current = controller; setGenerating(true); setError('')
    try {
      const result = await generateStudioVideo({ ...selection, prompt: prompt.trim(), sourceMediaLibraryAssetId: sourceAsset?.id || null, caption: brief?.caption || '', hashtags: brief?.hashtags || [], script: brief?.supportingCopy || '' }, controller.signal)
      setAsset(result); onToast(`Video created · ${result.creditsUsed} credits used.`)
    } catch (caught) { if (!(caught instanceof DOMException && caught.name === 'AbortError')) setError(caught instanceof Error ? caught.message : 'Video generation failed.') } finally { setGenerating(false); controllerRef.current = null }
  }
  ...
  return createPortal(<div className="fixed inset-0 z-[100] grid place-items-center bg-[#01070d]/92 p-2 backdrop-blur-xl sm:p-5">
    <div className="flex h-[min(900px,95vh)] w-full max-w-[1540px] flex-col overflow-hidden rounded-[30px] border border-brand-cyan/25 bg-[radial-gradient(circle_at_12%_12%,rgba(0,214,192,.08),transparent_27%),linear-gradient(145deg,#061824,#020b13)] shadow-[0_44px_160px_rgba(0,0,0,.74)]">
      <header className="flex min-h-16 items-center justify-between border-b border-border-soft px-4 sm:px-6"><div><span className="text-[8px] font-bold uppercase tracking-[.18em] text-brand-cyan">Generative Video</span><h2 className="mt-1 text-base font-bold">AI Video Studio <span className="font-medium text-text-soft">· Reel / Video</span></h2></div><div className="flex items-center gap-2"><span className="rounded-full border border-amber-400/25 bg-amber-400/[.06] px-3 py-1 text-[9px] font-bold text-amber-300">{estimating ? 'Calculating…' : `${credits} credits`}</span><button className="grid size-9 place-items-center rounded-xl border border-border-soft text-text-muted hover:text-white" onClick={onClose}><X className="size-4"/></button></div></header>
      <div className="grid min-h-0 flex-1 lg:grid-cols-[56%_44%]">
        ...
        <section className="flex min-h-0 flex-col bg-black/10 p-4 sm:p-5"><div className="min-h-0 flex-1 overflow-y-auto rounded-[24px] border border-border-soft bg-[radial-gradient(circle_at_50%_28%,rgba(0,214,192,.08),transparent_44%)] p-4">
          {!asset && !generating && <div className="grid h-full min-h-[480px] place-items-center text-center"><div><span className="mx-auto grid size-16 place-items-center rounded-[22px] border border-brand-cyan/25 bg-brand-cyan/10 text-brand-cyan"><Film className="size-7"/></span><h3 className="mt-4 text-lg font-bold">Video preview appears here.</h3><p className="mx-auto mt-2 max-w-sm text-[10px] leading-5 text-text-muted">Choose a generation model and explicit video settings. Use AI analyse & polish only when you want help turning a rough idea or URL into a production brief.</p>{selected && <div className="mx-auto mt-5 max-w-sm rounded-2xl border border-border-soft bg-black/15 p-3 text-left"><div className="flex items-center justify-between"><span className="text-[8px] uppercase tracking-[.13em] text-text-soft">Selected setup</span><span className="text-[8px] font-bold text-brand-cyan">{credits} credits</span></div><p className="mt-2 text-[10px] font-semibold">{selected.name} · {duration}s · {resolution} · {aspectRatio}</p><p className="mt-1 text-[8px] text-text-soft">{audio ? 'Native audio' : 'Silent'}{draft ? ' · Draft preview' : ''}{sourceAsset ? ' · Image reference' : ' · Text to video'}</p></div>}</div></div>}
          {generating && <div className="grid h-full min-h-[480px] place-items-center text-center"><div><LoaderCircle className="mx-auto size-10 animate-spin text-brand-cyan"/><h3 className="mt-4 text-base font-bold">Generating your video…</h3><p className="mt-2 text-[10px] text-text-muted">Video generation can take longer than image creation. Keep this window open; your reserved credits are protected if the render fails.</p></div></div>}
      ...
      <footer className="flex min-h-12 items-center justify-between border-t border-border-soft px-4 text-[8px] text-text-soft"><span>Model, duration, resolution and draft mode determine the video credit cost shown before generation.</span><Button size="sm" onClick={onClose}>Close</Button></footer>
    </div>
  </div>, document.body)
}
