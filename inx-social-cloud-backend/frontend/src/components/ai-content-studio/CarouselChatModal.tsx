...
import { ArrowLeft, ArrowRight, Bot, FileImage, Globe2, LoaderCircle, Paperclip, Save, Send, Sparkles, WandSparkles, X } from 'lucide-react'
...
function carouselCredits(slides: number) {
...
function buildDraft(asset: GeneratedAsset, brief: PostStudioBrief | null, messages: PostStudioMessage[], existing?: AIDraft | null): AIDraft {
  ...
  return {
    id: existing?.id || crypto.randomUUID(), contentType: 'carousel_post', title: (brief?.headline || prompt || 'AI carousel').slice(0, 70),
    thumbnailUrl: asset.thumbnailUrl || asset.slides?.[0]?.thumbnailUrl || asset.url, updatedAt: new Date().toISOString(), status: 'ready', prompt: prompt.slice(0, 1500),
    caption: asset.caption || brief?.caption || '', hashtags: asset.hashtags || brief?.hashtags || [], altText: '', asset,
...
export function CarouselChatModal({ open, type, access, initialDraft, onClose, onSaved, onContinue, onToast }: { open: boolean; type: AIContentType | null; access: AIPlanAccess; initialDraft?: AIDraft | null; onClose: () => void; onSaved: (draft: AIDraft) => void; onContinue: (draft: AIDraft) => void; onToast: (message: string) => void }) {
  ...
  const currentKey = useMemo(() => renderKey(brief, slides, platform, aspectRatio, sourceAnalysis), [brief, slides, platform, aspectRatio, sourceAnalysis])
  ...
  async function ask(nextMessages: PostStudioMessage[], nextUrls = urls, nextRefs = references) {
    ...
    try {
      ...
      const visible = nextMessages.filter((_, index) => index > 0).slice(-16)
      const result = await sendPostStudioMessage({ messages: [...(memory ? [memory] : []), formatInstruction, ...visible], urls: nextUrls, referenceAssetIds: nextRefs.map(item => item.id), platform, aspectRatio })
      setAssistantResult(result); setSourceAnalysis(result.sourceAnalysis || sourceAnalysis); setBrief(result.brief); setPlatform(result.brief.platform || platform); setAspectRatio((result.brief.aspectRatio === '1:1' || result.brief.aspectRatio === '4:5') ? result.brief.aspectRatio : aspectRatio)
  ...
  async function uploadReference(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const nextRefs = [{ id: stored.id, fileName: stored.fileName }, ...references.filter(item => item.id !== stored.id)].slice(0, 4); setReferences(nextRefs)
      const next = [...messages, { role: 'user' as const, content: `I uploaded “${stored.fileName}” as a carousel reference. Analyse it and use it where useful.` }]; setMessages(next)
  ...
  async function generate() {
    ...
    const controller = new AbortController(); controllerRef.current = controller; setGenerating(true); setError('')
    try {
      const prompt = messages.find(item => item.role === 'user')?.content || brief.objective
      const result = await generateConversationalCarousel({ prompt, platform, aspectRatio, slides, referenceAssetIds: references.map(item => item.id), brief: { ...brief, platform, aspectRatio }, sourceAnalysis }, controller.signal)
    ...
    } catch (caught) { if (!(caught instanceof DOMException && caught.name === 'AbortError')) setError(caught instanceof Error ? caught.message : 'Carousel generation failed.') } finally { setGenerating(false); controllerRef.current = null }
  }

  async function saveDraft() {
    if (!asset) return
    try { const draft = await saveAIDraft(buildDraft(asset, brief, messages, initialDraft)); onSaved(draft); onToast('Carousel saved to drafts.') } catch (caught) { setError(caught instanceof Error ? caught.message : 'Draft could not be saved.') }
  }

  async function continueToPosts() {
    if (!asset) return
    const draft = buildDraft(asset, brief, messages, initialDraft); onContinue(draft)
  }
  ...
  return createPortal(<div className="fixed inset-0 z-[100] grid place-items-center bg-[#01070d]/90 p-2 backdrop-blur-xl sm:p-5">
    <div className="flex h-[min(880px,94vh)] w-full max-w-[1540px] flex-col overflow-hidden rounded-[30px] border border-brand-cyan/25 bg-[radial-gradient(circle_at_12%_15%,rgba(0,214,192,.09),transparent_28%),linear-gradient(145deg,#061824,#020b13)] shadow-[0_44px_160px_rgba(0,0,0,.72)]">
      ...
      <div className="grid min-h-0 flex-1 lg:grid-cols-[47%_53%]">
        <section className="flex min-h-0 flex-col border-r border-border-soft">
          <div className="border-b border-border-soft px-4 py-3 sm:px-5"><h3 className="text-sm font-bold">Build the carousel with AI</h3><p className="mt-1 text-[9px] text-text-soft">Research, references and narrative planning happen here. Slides render only when you approve the brief.</p></div>
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4 sm:px-5">
            {messages.map((message, index) => <div key={`${index}-${message.content.slice(0, 18)}`} className={`flex gap-2 ${message.role === 'assistant' ? '' : 'justify-end'}`}>{message.role === 'assistant' && <span className="mt-1 grid size-7 shrink-0 place-items-center rounded-xl border border-brand-cyan/25 bg-brand-cyan/10 text-brand-cyan"><Bot className="size-3.5" /></span>}<div className={`max-w-[88%] rounded-2xl border px-3.5 py-3 text-[10px] leading-5 ${message.role === 'assistant' ? 'border-border-soft bg-white/[.035]' : 'border-brand-teal/30 bg-brand-teal/12 text-white'}`}><p className="whitespace-pre-wrap">{message.content}</p></div></div>)}
            ...
            {ready && renderNeeded && <div className="ml-9 rounded-[22px] border border-brand-green/30 bg-[linear-gradient(135deg,rgba(5,62,58,.55),rgba(4,25,35,.82))] p-5 shadow-[0_18px_60px_rgba(0,214,192,.08)]"><span className="text-[8px] font-bold uppercase tracking-[.17em] text-brand-green">Carousel ready</span><h4 className="mt-1 text-lg font-bold">Generate your {slides}-slide carousel</h4><p className="mt-2 text-[10px] leading-5 text-text-muted">The story, source context and visual direction are prepared. You can keep refining in chat or render the complete carousel now.</p><Button className="mt-4 min-h-11" variant="primary" disabled={insufficient || generating} onClick={() => void generate()}><WandSparkles className="size-4" />Generate carousel · {cost} credits</Button></div>}
            {error && <div className="rounded-xl border border-red-400/25 bg-red-500/[.07] px-3 py-2 text-[10px] text-red-200">{error}</div>}
          </div>
          <div className="space-y-2 border-t border-border-soft p-3 sm:p-4"><div className="overflow-hidden rounded-2xl border border-brand-cyan/25 bg-black/15"><textarea ref={inputRef} value={composer} onChange={event => setComposer(event.target.value)} onKeyDown={keyDown} rows={3} className="w-full resize-none bg-transparent px-3.5 py-3 text-[11px] outline-none placeholder:text-text-soft" placeholder="Describe the carousel, paste a URL, or ask to refine a slide/story…"/><div className="flex items-center justify-between border-t border-border-soft px-2.5 py-2"><div className="flex items-center gap-1"><input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={uploadReference}/><button onClick={() => fileRef.current?.click()} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-[9px] text-text-muted hover:bg-white/5"><Paperclip className="size-3.5" />Reference</button><button onClick={() => { setComposer(value => `${value}${value ? ' ' : ''}https://`); inputRef.current?.focus() }} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-[9px] text-text-muted hover:bg-white/5"><Globe2 className="size-3.5" />Add URL</button></div><button disabled={!composer.trim() || busy} onClick={() => void sendText()} className="grid size-9 place-items-center rounded-xl bg-brand-teal text-white disabled:opacity-40"><Send className="size-4" /></button></div></div>
            <details className="rounded-xl border border-border-soft bg-black/10"><summary className="cursor-pointer px-3 py-2 text-[9px] text-text-muted">Optional controls</summary><div className="grid gap-2 border-t border-border-soft p-3 sm:grid-cols-3"><label className="text-[8px] text-text-soft">Slides<select value={slides} onChange={event => setSlides(Number(event.target.value))} className="mt-1 w-full rounded-xl border border-border-soft bg-bg px-2 py-2 text-[10px] text-white">{[3,4,5,6,7,8,9,10].map(value => <option key={value}>{value}</option>)}</select></label><label className="text-[8px] text-text-soft">Platform<select value={platform} onChange={event => setPlatform(event.target.value)} className="mt-1 w-full rounded-xl border border-border-soft bg-bg px-2 py-2 text-[10px] text-white">{PLATFORMS.map(value => <option key={value}>{value}</option>)}</select></label><label className="text-[8px] text-text-soft">Aspect<select value={aspectRatio} onChange={event => setAspectRatio(event.target.value as PostStudioBrief['aspectRatio'])} className="mt-1 w-full rounded-xl border border-border-soft bg-bg px-2 py-2 text-[10px] text-white">{RATIOS.map(value => <option key={value}>{value}</option>)}</select></label></div></details></div>
        </section>
        <section className="flex min-h-0 flex-col bg-black/10 p-4 sm:p-5">
          <div className="min-h-0 flex-1 overflow-y-auto rounded-[24px] border border-border-soft bg-[radial-gradient(circle_at_50%_30%,rgba(0,214,192,.08),transparent_45%)] p-4">
            {!asset && !generating && <div className="grid h-full min-h-[420px] place-items-center text-center"><div><span className="mx-auto grid size-14 place-items-center rounded-2xl border border-brand-cyan/25 bg-brand-cyan/10 text-brand-cyan"><Sparkles className="size-6" /></span><h3 className="mt-4 text-lg font-bold">Your carousel comes together here.</h3><p className="mx-auto mt-2 max-w-md text-[10px] leading-5 text-text-muted">Plan the story in chat, review the research, then generate a coordinated sequence rather than unrelated images.</p></div></div>}
            {generating && <div className="grid h-full min-h-[420px] place-items-center text-center"><div><LoaderCircle className="mx-auto size-9 animate-spin text-brand-cyan"/><h3 className="mt-4 text-base font-bold">Designing all {slides} slides…</h3><p className="mt-2 text-[10px] text-text-muted">Keeping the story, typography and brand direction consistent across the full carousel.</p></div></div>}
            {asset && selectedSlide && <div className="space-y-4"><div className="relative mx-auto max-w-[620px] overflow-hidden rounded-2xl border border-border-soft bg-black/25"><img src={selectedSlide.url} alt={selectedSlide.altText || `Carousel slide ${activeSlide + 1}`} className="max-h-[610px] w-full object-contain"/></div><div className="flex items-center justify-between"><Button variant="secondary" disabled={activeSlide <= 0} onClick={() => setActiveSlide(value => Math.max(0, value - 1))}><ArrowLeft className="size-4"/>Previous</Button><span className="text-[10px] font-semibold">Slide {activeSlide + 1} / {slideAssets.length}</span><Button variant="secondary" disabled={activeSlide >= slideAssets.length - 1} onClick={() => setActiveSlide(value => Math.min(slideAssets.length - 1, value + 1))}>Next<ArrowRight className="size-4"/></Button></div><div className="flex gap-2 overflow-x-auto pb-1">{slideAssets.map((slide, index) => <button key={slide.id} onClick={() => setActiveSlide(index)} className={`h-20 w-20 shrink-0 overflow-hidden rounded-xl border ${index === activeSlide ? 'border-brand-cyan' : 'border-border-soft'}`}><img src={slide.thumbnailUrl || slide.url} className="h-full w-full object-cover"/></button>)}</div><div className="grid gap-3 md:grid-cols-[1fr_220px]"><div className="rounded-2xl border border-border-soft bg-black/15 p-4"><span className="text-[8px] font-bold uppercase tracking-[.13em] text-text-soft">Caption</span><p className="mt-2 whitespace-pre-wrap text-[10px] leading-5 text-text-main">{asset.caption}</p></div><div className="rounded-2xl border border-border-soft bg-black/15 p-4"><span className="text-[8px] font-bold uppercase tracking-[.13em] text-text-soft">Hashtags</span><p className="mt-2 text-[9px] leading-5 text-text-muted">{asset.hashtags?.map(tag => `#${tag.replace(/^#/, '')}`).join(' ')}</p></div></div></div>}
          </div>
          {asset && <div className="mt-3 grid gap-2 sm:grid-cols-2"><Button onClick={() => void saveDraft()}><Save className="size-4"/>Save draft</Button><Button variant="primary" onClick={() => void continueToPosts()}>Continue to Posts<ArrowRight className="size-4"/></Button></div>}
        </section>
      </div>
      <footer className="flex min-h-12 items-center justify-between border-t border-border-soft px-4 text-[8px] text-text-soft"><span>Chat and source analysis are included. Credits are used only when the final carousel completes.</span><Button size="sm" onClick={onClose}>Close</Button></footer>
    </div>
  </div>, document.body)
}
