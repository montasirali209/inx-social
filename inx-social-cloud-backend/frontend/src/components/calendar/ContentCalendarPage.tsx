...
function CalendarSkeleton() {
  ...
  return <div aria-label="Loading Content Calendar" className="space-y-4" role="status">
    <div className="grid grid-cols-2 gap-2 sm:gap-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">{stats.map(([emoji, label], index) => <div className="min-h-28 min-w-0 rounded-card border border-border-soft bg-panel/70 p-3 sm:p-4" key={label}><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl border border-white/[.07] bg-white/[.025] text-lg motion-safe:animate-bounce" style={{ animationDelay: `${index * 80}ms` }}>{emoji}</span><span><small className="block text-[10px] text-text-muted">{label}</small><strong className="mt-1 block text-sm">Updating…</strong></span></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-border-soft"><span className="block h-full w-2/3 animate-pulse rounded-full bg-gradient-to-r from-brand-teal/60 to-brand-cyan motion-reduce:animate-none" /></div></div>)}</div>
...
export function ContentCalendarPage() {
  ...
  return <div className="dashboard-canvas">
    <section aria-label="Calendar publishing status" className="mb-4 grid grid-cols-2 items-stretch gap-2 pb-2 sm:gap-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">{calendarStats.map((stat, index) => <CalendarStatCard icon={statIcons[index]} key={stat.label} onClick={stat.label === 'Needs Review' && stat.value > 0 ? openNeedsReview : undefined} stat={stat} />)}</section>
