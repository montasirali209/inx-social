...
export function CalendarQuickActionsCard({ date, time }: { date: string; time: string }) {
  const scheduleUrl = `/app/posts&scheduleDate=${encodeURIComponent(date)}${time ? `&scheduleTime=${encodeURIComponent(time)}` : ''}`
  const actions = [
    { label: 'Schedule Content', href: scheduleUrl, icon: PlusCircle },
    { label: 'Create Draft', href: '/app/posts&draft=true', icon: FilePenLine },
    { label: 'Upload Media', href: '/app/media-library', icon: ImagePlus },
