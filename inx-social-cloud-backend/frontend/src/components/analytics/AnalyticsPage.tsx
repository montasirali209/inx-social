...
import { saveFacebookDemographicsSnapshot } from '../../lib/dashboard-api'
...
export function AnalyticsPage() {
  ...
  async function saveSnapshot(input: { capturedAt: string; audienceSize: number | null; ageGender: Array<{ age: string; women: number; men: number; unknown: number }> }) {
    if (!selected || selected.platform !== 'facebook') return
    await saveFacebookDemographicsSnapshot({ connectedPageId: selected.id, ...input })
    const data = await fetchAnalyticsForSource(selected, days, true)
    queryClient.setQueryData(queryKey, data)
  }
  ...
  return <div className="analytics-fluid-canvas dashboard-canvas space-y-4 pb-8">
    ...
    {view && <div className="analytics-data-transition space-y-4" key={`${selectedKey}-${days}`}>
      ...
      {activeTab === 'overview' && <><div className="grid items-stretch gap-4 xl:grid-cols-[minmax(0,2fr)_minmax(320px,.72fr)]"><PerformanceOverTimeCard interval={interval} points={chartPoints} setInterval={setInterval} /><EngagementByPlatformCard platform={view.source.platform} total={view.totalEngagements} /></div><div className="grid items-stretch gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(260px,.75fr)_minmax(260px,.75fr)]"><TopPerformingPostsCard onViewAll={() => setActiveTab('content_performance')} posts={view.topPosts} /><AudienceGrowthCard points={view.performance} total={view.audienceGrowth} /><AudienceDemographicsCard capability={view.source.capabilities?.instagramDemographics} demographics={view.source.demographics} onEditSnapshot={() => setSnapshotOpen(true)} platform={view.source.platform} /></div><BestTimeToPostCard cells={view.heatmap} /></>}
      ...
      {activeTab === 'audience' && <div className="grid gap-4 lg:grid-cols-2"><AudienceGrowthCard points={view.performance} total={view.audienceGrowth} /><AudienceDemographicsCard capability={view.source.capabilities?.instagramDemographics} demographics={view.source.demographics} onEditSnapshot={() => setSnapshotOpen(true)} platform={view.source.platform} /></div>}
    ...
    {snapshotOpen && selected?.platform === 'facebook' && <FacebookDemographicsSnapshotModal onClose={() => setSnapshotOpen(false)} onSave={saveSnapshot} pageName={selected.displayName} snapshot={view?.source.demographics?.facebookSnapshot || null} />}
  </div>
}
