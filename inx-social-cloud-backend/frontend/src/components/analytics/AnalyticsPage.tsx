...
export function AnalyticsPage() {
  ...
  return <div className="analytics-fluid-canvas dashboard-canvas space-y-4 pb-8">
    ...
    {view && <div className="analytics-data-transition space-y-4" key={`${selectedKey}-${days}`}>
      ...
      {activeTab === 'overview' && <><div className="grid items-stretch gap-4 xl:grid-cols-[minmax(0,2fr)_minmax(320px,.72fr)]"><PerformanceOverTimeCard interval={interval} points={chartPoints} setInterval={setInterval} /><EngagementByPlatformCard platform={view.source.platform} total={view.totalEngagements} /></div><div className="grid items-stretch gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(260px,.75fr)_minmax(300px,.9fr)]"><TopPerformingPostsCard onViewAll={() => setActiveTab('content_performance')} platform={view.source.platform} posts={view.topPosts} /><AudienceGrowthCard points={view.performance} total={view.audienceGrowth} />{audiencePulse}</div><BestTimeToPostCard cells={view.heatmap} /></>}
      {activeTab === 'content_performance' && <div className="grid gap-4 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,.75fr)]"><PerformanceOverTimeCard interval={interval} points={chartPoints} setInterval={setInterval} /><TopPerformingPostsCard onViewAll={() => {}} platform={view.source.platform} posts={view.topPosts} /></div>}
      ...
      {activeTab === 'videos' && <TopPerformingPostsCard onViewAll={() => {}} platform={view.source.platform} posts={view.topPosts.filter(post => /video|reel/i.test(post.contentType))} />}
