...
export function AnalyticsPage() {
  ...
  return <div className="analytics-fluid-canvas dashboard-canvas space-y-4 pb-8">
    ...
    {analytics.isLoading && !analytics.data && <AnalyticsSkeleton />}
    {analytics.isError && analytics.data ? <div className="rounded-xl border border-brand-amber/20 bg-brand-amber/8 px-4 py-3 text-[11px] text-brand-amber"><strong>Live refresh temporarily paused.</strong> INXSocial is keeping the most recent verified analytics already loaded in this workspace. {analytics.error instanceof Error ? analytics.error.message : 'The live source will retry automatically.'}</div> : null}
    {analytics.isError && !analytics.data && <div className="rounded-panel border border-brand-red/25 bg-brand-red/8 p-6"><h2 className="font-semibold">Analytics could not be loaded</h2><p className="mt-2 text-xs leading-5 text-text-muted">{analytics.error instanceof Error ? analytics.error.message : 'Reconnect this account or try again.'}</p><button className="mt-4 inline-flex min-h-10 items-center rounded-xl border border-border-soft px-4 text-xs" onClick={() => void analytics.refetch()} type="button">Retry analytics</button></div>}
