...
function linePoints(points: PerformancePoint[], key: SeriesKey, maximum: number) {
  return points.map((point, index) => `${plot.left + index / Math.max(1, points.length - 1) * (plot.right - plot.left)},${plot.bottom - point[key] / maximum * (plot.bottom - plot.top)}`).join(' ')
}
...
export function PerformanceOverTimeCard({ points, interval, setInterval }: { points: PerformancePoint[]; interval: 'daily' | 'weekly' | 'monthly'; setInterval: (value: 'daily' | 'weekly' | 'monthly') => void }) {
  ...
  return <AnalyticsCard className="xl:col-span-2">
    ...
    <div className="relative px-3 sm:px-5">{activePoint && <div className="pointer-events-none absolute right-6 top-2 z-10 rounded-xl border border-brand-cyan/20 bg-bg/95 p-3 text-[10px] shadow-panel"><strong>{activePoint.label}</strong>{series.map((item) => <span className="mt-1 flex items-center justify-between gap-8 text-text-muted" key={item.key}><span><i className="mr-1.5 inline-block size-2 rounded-full" style={{ backgroundColor: item.colour }} />{item.label}</span><b className="text-white">{formatAnalyticsValue(activePoint[item.key], 'compact')}</b></span>)}</div>}
      <svg aria-label="Performance over time chart" className="h-[260px] w-full touch-pan-y sm:h-[310px]" onPointerLeave={() => setActive(null)} onPointerMove={track} preserveAspectRatio="none" role="img" viewBox="0 0 1000 275">
        ...
        {series.map((item) => <g key={item.key}><polyline fill="none" opacity=".18" points={linePoints(points, item.key, maximum)} stroke={item.colour} strokeLinecap="round" strokeLinejoin="round" strokeWidth="6" /><polyline fill="none" points={linePoints(points, item.key, maximum)} stroke={item.colour} strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" /></g>)}
        {active !== null && <line stroke="rgba(45,212,191,.45)" strokeDasharray="4 4" x1={plot.left + active / Math.max(1, points.length - 1) * (plot.right - plot.left)} x2={plot.left + active / Math.max(1, points.length - 1) * (plot.right - plot.left)} y1={plot.top} y2={plot.bottom} />}
        {points.map((point, index) => index % labelEvery === 0 || index === points.length - 1 ? <text fill="#64748b" fontSize="10" key={point.date} textAnchor={index === 0 ? 'start' : index === points.length - 1 ? 'end' : 'middle'} x={plot.left + index / Math.max(1, points.length - 1) * (plot.right - plot.left)} y="262">{point.label}</text> : null)}
      </svg>
