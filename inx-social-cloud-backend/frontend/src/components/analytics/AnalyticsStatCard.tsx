import { Activity, Eye, Heart, Link2, MessageCircle, Send, UsersRound, type LucideIcon } from 'lucide-react'
import { useEffect, useState } from 'react'
import { formatAnalyticsValue } from '../../data/analyticsData'
import type { AnalyticsStat } from '../../types/analytics'

const icons: Record<string, LucideIcon> = { followers: UsersRound, views: Eye, reactions: Heart, comments: MessageCircle, 'avg-interactions': Activity, 'engagement-rate': Heart, interactions: Activity, clicks: Link2, 'engaged-posts': Heart, posts: Send }
...
export function AnalyticsStatCard({ stat }: { stat: AnalyticsStat }) {
  ...
  return <article className="analytics-stat-card interactive-surface group min-w-0 rounded-card border p-4 focus-within:border-brand-cyan/40"><div className="flex items-start gap-3"><span className={`grid size-10 shrink-0 place-items-center rounded-xl border ${toneClasses[stat.tone]}`}><Icon className="size-5" /></span><div className="min-w-0 flex-1"><p className="truncate text-[11px] text-text-muted">{stat.label}</p>{stat.value === null ? <strong className="analytics-kpi-value mt-1 block text-sm tracking-tight text-text-soft">Unavailable</strong> : <AnimatedNumber format={stat.format} value={stat.value} />}<p className="mt-1 truncate text-[9px] text-text-soft" title={stat.availability || stat.detail}>{stat.detail}</p></div></div><Sparkline colour={colours[stat.tone]} values={stat.sparkline} /></article>
}
