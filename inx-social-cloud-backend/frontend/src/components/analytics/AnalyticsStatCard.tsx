...
function AnimatedValue({ stat }: { stat: AnalyticsStat }) {
  const [value, setValue] = useState(stat.value === null ? null : 0)
  useEffect(() => {
    if (stat.value === null || window.matchMedia('(prefers-reduced-motion: reduce)').matches) { setValue(stat.value); return }
