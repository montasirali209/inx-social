...
async function downloadSelectedUgcAvatars(){
  ...
  try{
    ...
    link.href=url;
    link.download=match?.[1]||'inx-ugc-creators.zip';
...
async function deleteSelectedUgcAvatars(){
  ...
  if(!window.confirm(`Remove ${ids.length} creator${ids.length===1?'':'s'} from UGC Studio?\n\n${preview}\n\nThey will disappear from Browse creators for customers. Existing campaigns that already reference these creators will remain intact.`))return;
