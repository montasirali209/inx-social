...
test('Stock Video Creator UI resumes active jobs and hands completed video to Posts', () => {
  ...
  assert.match(productionRail, /Your Generated Videos/);
  assert.match(productionRail, /status === 'completed'/);
  assert.match(productionRail, /kind\?: VideoProductionKind/);
  assert.match(productionRail, /Select one to preview and play it here/);
  assert.match(productionRail, /View in Media Library/);
  assert.match(productionRail, /thumbnailUrl/);
  assert.match(component, /Dismiss error/);
  assert.match(component, /Selected generated video/);
  assert.match(component, /kind="stock"/);
  assert.match(productionRail, /2xl:w-\[250px\]/);
