...
test('Analytics uses live Post for Me platform data and derives transparent metrics without mock values', () => {
  ...
  assert.match(service, /ANALYTICS_PARTIAL_RETRY_AFTER_MS = 15 \* 1000/);
  ...
  assert.match(page, /data\?\.analytics\?\.provider\?\.repairRequired/);
  assert.match(page, /states\.includes\('partial'\)/);
  assert.match(page, /return 15_000/);
  assert.match(page, /forcedSelections/);
  assert.match(page, /loadAnalytics\(force\)/);
  assert.match(page, /repairAnalyticsAccess/);
  assert.match(page, /connectPostForMePlatform\(selectedAccount\.platform\)/);
