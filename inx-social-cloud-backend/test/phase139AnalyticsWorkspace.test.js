...
test('Analytics uses live Post for Me platform data and derives transparent metrics without mock values', () => {
  ...
  assert.match(service, /ANALYTICS_EMPTY_FEED_RETRY_AFTER_MS = 60 \* 1000/);
  assert.match(service, /ANALYTICS_FORCE_EMPTY_RETRY_DELAYS_MS/);
  assert.match(service, /options\.forceRefresh/);
  assert.match(service, /value\?\.provider\?\.sourceState !== 'feed_empty'/);
  ...
  assert.doesNotMatch(service, /\['instagram', 'facebook', 'x'\]\.includes\(profile\.platform\)/);
  ...
  assert.doesNotMatch(service, /persisted\?\.provider\?\.repairRequired === true/);
  ...
  assert.match(page, /resolvedSelectionToken/);
  assert.match(page, /setResolvedSelectionToken\(selection\)/);
  assert.match(page, /const selectedIsSyncing = !view \|\| manualRefreshing \|\|/);
  assert.match(page, /providerSourceState === 'ownership_mismatch'/);
  assert.match(page, /X returned a feed that could not be verified for this account/);
  assert.match(page, /you do not need to reconnect the account again/);
  ...
  assert.doesNotMatch(page, /Refresh \$\{platformNames\[selectedAccount!\.platform as Platform\]\} analytics access/);
  assert.match(page, /AnalyticsWorkspaceSkeleton active=\{selectedIsSyncing\}/);
  assert.match(page, /isPending=\{partialMetrics && !selectedIsSyncing\}/);
